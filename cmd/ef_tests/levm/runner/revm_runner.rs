...
pub fn re_run_failed_ef_test_multiple_forks(
    test: &EFTest,
...
pub fn prepare_revm_for_tx<'state>(
    ...
    */
    ...
    let evm_builder = Revm::builder()
        ...
        // .with_spec_id(test.fork()) // here we should pass the fork
        .with_spec_id(fork) // here we should pass the fork
...
pub fn _run_ef_test_revm_multiple_forks(test: &EFTest) -> Result<EFTestReport, EFTestRunnerError> {
    let hash = test
        ._info
        .generated_test_hash
        .or(test._info.hash)

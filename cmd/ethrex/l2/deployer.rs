...
use ethrex_l2_sdk::{
    build_generic_tx,
    calldata::{encode_calldata, encode_tuple},
    create2_deploy_from_bytecode_no_wait, initialize_contract_no_wait, send_generic_transaction,
...
/// Bytecode of the UpgradeTimelock contract.
/// This is generated by the [build script](./build.rs).
const UPGRADE_TIMELOCK_BYTECODE: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/contracts/solc_out/UpgradeTimelock.bytecode"
));
...
const INITIALIZE_ON_CHAIN_PROPOSER_SIGNATURE_BASED: &str = "initialize(bool,address,address,bool,bool,bool,bool,address,address,address,address,bytes32,bytes32,bytes32,address,uint256)";
const INITIALIZE_ON_CHAIN_PROPOSER_SIGNATURE: &str = "initialize(bool,address,address,bool,bool,bool,bool,address,address,address,address,bytes32,bytes32,bytes32,address[],uint256)";
...
pub struct ContractAddresses {
    ...
    pub on_chain_proposer_upgrade_timelock_address: Address,
...
async fn deploy_contracts(
    ...
    info!("Deploying UpgradeTimelock");

    if UPGRADE_TIMELOCK_BYTECODE.is_empty() {
        return Err(DeployerError::BytecodeNotFound);
    }

    let upgrade_timelock_constructor_args = encode_tuple(&[
        Value::Address(opts.on_chain_proposer_owner),
        Value::Address(opts.on_chain_proposer_owner),
    ])?;

    let (upgrade_timelock_deployment_tx_hash, upgrade_timelock_address) =
        create2_deploy_from_bytecode_no_wait(
            &upgrade_timelock_constructor_args,
            UPGRADE_TIMELOCK_BYTECODE,
            deployer,
            &salt,
            eth_client,
            Overrides {
    ...
    info!(
        "UpgradeTimelock deployed:\n  address={:#x}, tx_hash={:#x}",
        upgrade_timelock_address, upgrade_timelock_deployment_tx_hash
    );
    ...
    trace!(
        upgrade_timelock_address = ?upgrade_timelock_address,
    ...
    let receipts = vec![
        upgrade_timelock_deployment_tx_hash,
        on_chain_proposer_deployment.implementation_tx_hash,
        on_chain_proposer_deployment.proxy_tx_hash,
        bridge_deployment.implementation_tx_hash,
        bridge_deployment.proxy_tx_hash,
    ...
    Ok((
        ContractAddresses {
            ...
            on_chain_proposer_upgrade_timelock_address: upgrade_timelock_address,
...
async fn initialize_contracts(
    ...
    if opts.deploy_based_contracts {
        ...
        let calldata_values = vec![
            ...
            Value::Address(contract_addresses.on_chain_proposer_upgrade_timelock_address),
...
fn write_contract_addresses_to_env(
    ...
    writeln!(
        writer,
        "ETHREX_DEPLOYER_ON_CHAIN_PROPOSER_UPGRADE_TIMELOCK_ADDRESS={:#x}",
        contract_addresses.on_chain_proposer_upgrade_timelock_address
    )?;
    writeln!(
        writer,

...
pub struct DeployerOptions {
    ...
    #[arg(
        long,
        value_name = "ADDRESS",
        env = "ETHREX_SETTLEMENT_OWNER",
        help_heading = "Deployer options",
        help = "Address of the owner of the Settlement contract, who can upgrade the contract."
    )]
    pub settlement_owner: Address,
    #[arg(
        long,
        value_name = "ADDRESS",
    ...
    #[arg(
        long,
        value_name = "PRIVATE_KEY",
        env = "ETHREX_SETTLEMENT_OWNER_PK",
        help_heading = "Deployer options",
        help = "Private key of the owner of the Settlement contract. If set, the deployer will send a transaction to accept the ownership.",
        requires = "settlement_owner"
    )]
    pub settlement_owner_pk: Option<SecretKey>,
    #[arg(
        long,
        value_name = "PATH",
    ...
    #[arg(
        ...
        action = ArgAction::SetTrue,
        help_heading = "Deployer options",
        help = "If set to true, it will deploy the SequencerRegistry contract and a modified Settlement contract."
    )]
    ...
    #[arg(
        long,
        default_value = "3000",
        env = "ETHREX_SETTLEMENT_INCLUSION_MAX_WAIT",
        help_heading = "Deployer options",
...
/// Bytecode of the Settlement contract.
/// This is generated by the [build script](./build.rs).
const SETTLEMENT_BYTECODE: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/contracts/solc_out/Settlement.bytecode"
));
...
/// Bytecode of the based Settlement contract.
/// This is generated by the [build script](./build.rs).
const SETTLEMENT_BASED_BYTECODE: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/contracts/solc_out/SettlementBased.bytecode"
));
...
const INITIALIZE_SETTLEMENT_SIGNATURE_BASED: &str = "initialize(bool,address,bool,bool,bool,bool,address,address,address,address,bytes32,bytes32,bytes32,address,uint256)";
const INITIALIZE_SETTLEMENT_SIGNATURE: &str = "initialize(bool,address,bool,bool,bool,bool,address,address,address,address,bytes32,bytes32,bytes32,address[],uint256)";
...
async fn deploy_contracts(
    ...
    info!("Deploying Settlement");

    trace!("Attempting to deploy Settlement contract");
    let bytecode = if opts.deploy_based_contracts {
        SETTLEMENT_BASED_BYTECODE.to_vec()
    } else {
        SETTLEMENT_BYTECODE.to_vec()
    };
    ...
    let settlement_deployment = deploy_with_proxy_from_bytecode_no_wait(
        deployer,
        eth_client,
        &bytecode,
        &salt,
    ...
    info!(
        "Settlement deployed:\n  Proxy -> address={:#x}, tx_hash={:#x}\n  Impl  -> address={:#x}, tx_hash={:#x}",
        settlement_deployment.proxy_address,
        settlement_deployment.proxy_tx_hash,
        settlement_deployment.implementation_address,
        settlement_deployment.implementation_tx_hash,
    );
    ...
    let tdx_verifier_address = match opts.tdx_verifier_address {
        ...
        None if opts.tdx => {
            ...
            let tdx_verifier_address =
                deploy_tdx_contracts(opts, settlement_deployment.proxy_address)?;
    ...
    trace!(
        settlement_proxy_address = ?settlement_deployment.proxy_address,
        bridge_proxy_address = ?bridge_deployment.proxy_address,
        settlement_implementation_address = ?settlement_deployment.implementation_address,
    ...
    Ok((
        ContractAddresses {
            settlement_address: settlement_deployment.proxy_address,
...
fn deploy_tdx_contracts(
    ...
    Command::new("make")
        ...
        .env("SETTLEMENT", format!("{settlement:#x}"))
...
async fn initialize_contracts(
    ...
    trace!(committer_l1_address = %opts.committer_l1_address, "Using committer L1 address for Settlement initialization");

    info!("Reading verification keys for Settlement initialization");
    ...
    info!("Initializing Settlement");

    if opts.deploy_based_contracts {
        // Initialize Settlement with Based config and SequencerRegistry
        ...
        trace!(calldata_values = ?calldata_values, "Settlement initialization calldata values");
        let settlement_initialization_calldata =
            encode_calldata(INITIALIZE_SETTLEMENT_SIGNATURE_BASED, &calldata_values)?;
        ...
        let initialize_tx_hash = initialize_contract_no_wait(
            contract_addresses.settlement_address,
            settlement_initialization_calldata,
            &deployer,
            eth_client,
            Overrides {
        ...
        info!(tx_hash = %format!("{initialize_tx_hash:#x}"), "Settlement initialized");
        ...
        let initialize_tx_hash = {
            ...
            let calldata_values = vec![
                ...
                Value::Address(contract_addresses.settlement_address),
            ];
        ...
        // Initialize only Settlement without Based config
        let calldata_values = vec![
            Value::Bool(opts.validium),
            Value::Address(deployer_address),
            Value::Bool(opts.risc0),
        ...
        trace!(calldata_values = ?calldata_values, "Settlement initialization calldata values");
        let settlement_initialization_calldata =
            encode_calldata(INITIALIZE_SETTLEMENT_SIGNATURE, &calldata_values)?;
        let initializer_nonce = eth_client
            .get_nonce(
                initializer.address(),
                BlockIdentifier::Tag(BlockTag::Pending),
    ...
    info!(
        ...
        "Settlement bridge address initialized"
    );

    tx_hashes.push(initialize_bridge_address_tx_hash);

    if opts.settlement_owner != initializer.address() {
        let initializer_nonce = eth_client
            .get_nonce(
                initializer.address(),
                BlockIdentifier::Tag(BlockTag::Pending),
        ...
        let transfer_ownership_tx_hash = {
            let owner_transfer_calldata = encode_calldata(
                ...
                &[Value::Address(opts.settlement_owner)],
            )?;
        ...
        if let Some(owner_pk) = opts.settlement_owner_pk {
            ...
            info!(
                ...
                "Settlement ownership transfered"
            );
        } else {
            info!(
                transfer_tx_hash = %format!("{transfer_ownership_tx_hash:#x}"),
                "Settlement ownership transfered but not accepted yet"
            );
        }
    }
...
fn write_contract_addresses_to_env(
    ...
    writeln!(
        writer,
        "ETHREX_COMMITTER_SETTLEMENT_ADDRESS={:#x}",
        contract_addresses.settlement_address
    )?;
    writeln!(
        writer,

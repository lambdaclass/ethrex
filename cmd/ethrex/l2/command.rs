...
fn set_if_none<T: Clone>(target: &mut Option<T>, default: &Option<T>) {
    if let (None, Some(value)) = (target.as_ref(), default) {
        *target = Some(value.clone());
    }
}

fn populate_missing_l2_defaults(options: &mut Options) {
    let defaults = Options::default();
    set_if_none(
        &mut options.sponsorable_addresses_file_path,
        &defaults.sponsorable_addresses_file_path,
    );

    let sequencer_defaults = &defaults.sequencer_opts;
    let sequencer_options = &mut options.sequencer_opts;

    // Eth options
    if sequencer_options.eth_opts.rpc_url.is_empty() {
        sequencer_options.eth_opts.rpc_url = sequencer_defaults.eth_opts.rpc_url.clone();
    }

    // Watcher options
    set_if_none(
        &mut sequencer_options.watcher_opts.bridge_address,
        &sequencer_defaults.watcher_opts.bridge_address,
    );

    // Block producer
    set_if_none(
        &mut sequencer_options.block_producer_opts.coinbase_address,
        &sequencer_defaults.block_producer_opts.coinbase_address,
    );

    // Committer options - Here is a problem if we want to differ from remote and local signing - always set local
    set_if_none(
        &mut sequencer_options.committer_opts.committer_l1_private_key,
        &sequencer_defaults.committer_opts.committer_l1_private_key,
    );
    set_if_none(
        &mut sequencer_options.committer_opts.committer_remote_signer_url,
        &sequencer_defaults
            .committer_opts
            .committer_remote_signer_url,
    );
    set_if_none(
        &mut sequencer_options
            .committer_opts
    ...
    set_if_none(
        &mut sequencer_options.committer_opts.on_chain_proposer_address,
        &sequencer_defaults.committer_opts.on_chain_proposer_address,
    );
    set_if_none(
        &mut sequencer_options.committer_opts.batch_gas_limit,
        &sequencer_defaults.committer_opts.batch_gas_limit,
    );
    set_if_none(
        &mut sequencer_options.committer_opts.first_wake_up_time_ms,
        &sequencer_defaults.committer_opts.first_wake_up_time_ms,
    );
    ...
    if let (None, Some(default_path)) = (
        ...
        sequencer_options
            .proof_coordinator_opts
            .proof_coordinator_qpl_tool_path = Some(default_path.clone());
    }
    set_if_none(
        &mut sequencer_options.proof_coordinator_opts.remote_signer_url,
        &sequencer_defaults.proof_coordinator_opts.remote_signer_url,
    );
    set_if_none(
        &mut sequencer_options
            .proof_coordinator_opts
    ...
    // Aligned options
    set_if_none(
        &mut sequencer_options.aligned_opts.beacon_url,
        &sequencer_defaults.aligned_opts.beacon_url,
    );
    set_if_none(
        &mut sequencer_options.aligned_opts.aligned_network,
        &sequencer_defaults.aligned_opts.aligned_network,
    );
    set_if_none(
        &mut sequencer_options.aligned_opts.aligned_sp1_elf_path,
        &sequencer_defaults.aligned_opts.aligned_sp1_elf_path,
    );
}
...
impl L2Command {
    pub async fn run(self) -> eyre::Result<()> {
        ...
        if l2_options.node_opts.dev {
            ...
            populate_missing_l2_defaults(&mut l2_options);
            l2_options
                .sequencer_opts
                .committer_opts

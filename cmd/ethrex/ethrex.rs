...
async fn main() -> eyre::Result<()> {
    ...
    let (datadir, cancel_token, peer_table, shared_local_node, store, pruner_handle) =
    ...
    tokio::select! {
        _ = tokio::signal::ctrl_c() => {
            server_shutdown(&datadir, &cancel_token, peer_table, shared_local_node, &store, pruner_handle.take()).await;
        }

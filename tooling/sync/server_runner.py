...
def parse_args():
    parser = argparse.ArgumentParser(description="Run a Makefile with optional variables.")
    parser.add_argument("--snap", action="store_true", help="Whether snap is activated")
    parser.add_argument("--healing", action="store_true", help="Whether healing is activated")
    parser.add_argument("--memory", action="store_true", help="Whether memory is activated")
    parser.add_argument("--network", type=str, default="hoodi", help="Network variable (default: hoodi)")
    parser.add_argument("--branch", type=str, default="snap_sync", help="Branch variable (default: snap_sync)")
    parser.add_argument("--logs_file", type=str, default="output", help="Logs file name (default: output)")
...
def main():
    ...
    try:
        while True:
            ...
            subprocess.run(command + [f"LOGS_FILE={logs_file}_{start_time}.log"], check=True)
            while True:
                try:

...
/// Start a mock JSON-RPC server that returns canned responses based on method.
...
fn make_repl(endpoint: &str) -> Repl {
    Repl::new(
        RpcClient::new(endpoint.to_string()),
...
async fn test_eth_block_number() {
    let (endpoint, handle) = start_mock_server().await;
    let repl = make_repl(&endpoint);

    let result = repl.execute_command("eth.blockNumber").await;
    // 0x10d4f = 68943 in decimal
...
async fn test_eth_chain_id() {
    ...
    let result = repl.execute_command("eth.chainId").await;
...
async fn test_eth_gas_price() {
    ...
    let result = repl.execute_command("eth.gasPrice").await;
...
async fn test_web3_client_version() {
    ...
    let result = repl.execute_command("web3.clientVersion").await;
    assert!(
...
async fn test_net_version() {
    ...
    let result = repl.execute_command("net.version").await;
    assert!(result.contains("1"), "expected '1', got: {result}");
...
async fn test_net_peer_count() {
    ...
    let result = repl.execute_command("net.peerCount").await;
    // 0x19 = 25
...
async fn test_unknown_command() {
    ...
    let result = repl.execute_command("foo.bar").await;
    assert!(
...
async fn test_missing_required_arg() {
    ...
    let result = repl.execute_command("eth.getBalance").await;
    assert!(
...
async fn test_invalid_address() {
    ...
    let result = repl.execute_command("eth.getBalance 0xinvalid").await;
    assert!(
...
async fn test_block_number_decimal_conversion() {
    ...
    // Decimal 100 should be converted to 0x64 and sent as block id
    let result = repl.execute_command("eth.getBlockByNumber 100 false").await;
    // The mock returns null for getBlockByNumber, which formats as "null"
    assert!(
...
async fn test_utility_to_wei() {
    let (_endpoint, handle) = start_mock_server().await;
    let repl = make_repl(&_endpoint);

    let result = repl.execute_command("toWei 1 ether").await;
    assert!(
        result.contains("1000000000000000000"),
...
async fn test_utility_from_wei() {
    ...
    let result = repl.execute_command("fromWei 1000000000000000000 ether").await;
    assert!(result.contains("1"), "expected 1, got: {result}");
...
async fn test_utility_to_hex() {
    ...
    let result = repl.execute_command("toHex 255").await;
...
async fn test_utility_from_hex() {
    ...
    let result = repl.execute_command("fromHex 0xff").await;
...
async fn test_builtin_in_non_interactive() {
    ...
    let result = repl.execute_command(".help").await;
    assert!(
...
async fn test_empty_input() {
    ...
    let result = repl.execute_command("").await;

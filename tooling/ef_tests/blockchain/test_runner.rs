...
/// drift on EIP-8037 state-gas accounting, EIP-7928 BAL construction,
/// receipts/state/requests roots, and bloom.
///
...
async fn run_builder_parity(test_key: &str, test: &TestUnit) -> Result<(), String> {
    ...
    for block_fixture in test.blocks.iter() {
        ...
        // next iteration can use it as parent. Using the expected block keeps
        // BAL recorder + storage state in lockstep with the validator path.

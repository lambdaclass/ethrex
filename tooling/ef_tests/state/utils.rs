...
pub async fn load_initial_state(test: &EFTest) -> (EvmState, BlockHash, Store) {
    ...
    let vm_db: DynVmDatabase = Box::new(StoreVmDatabase::new(storage.clone(), block_hash));

    (evm_state(vm_db), block_hash, storage)
}

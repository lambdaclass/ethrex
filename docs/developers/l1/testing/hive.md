...
- `tooling/ef_tests/engine/.fixtures_url_focil` — EIP-7805 (FOCIL) fixtures, filled on the "Bogota" network. The bundle is filled with `--generate-all-formats` and ships six format trees, but only `blockchain_tests_engine/for_bogota` is extracted: the inclusion-list variants that carry the FOCIL coverage exist only in the engine formats, and every one of the 3,092 paths under `blockchain_tests/for_bogota` also appears under `blockchain_tests_engine/for_bogota` -- the same test bodies delivered as RLP blocks instead of through `engine_newPayload`. `state_tests/for_bogota` overlaps the engine tree the same way apart from seven state-test-only cases (blob fee cap, `BLOCKHASH` recency, sender balance), none of which carries an inclusion list. Running those additionally would need a `Bogota` fork in the blockchain and state runners, which is a question about what this client's post-Amsterdam fork contains rather than about FOCIL. The bundle must be filled on the same Amsterdam that `.fixtures_url_amsterdam` pins.

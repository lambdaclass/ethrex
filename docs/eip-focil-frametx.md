...
* [EIP-8250](./eip-8250.md) keyed nonce state at `(tx.sender, nonce_key)` for every nonzero key in `tx.nonce_keys`;
...
Replay MUST count each distinct `codeHash` once and sum each distinct code body's byte length once. An account with empty code is not a body and costs nothing.

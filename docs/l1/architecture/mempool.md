...
**Blob count may not shrink.** Independently of the fee comparison, a replacement may not reduce the number of blobs the pooled transaction carries: `new.blob_versioned_hashes.len() < old.blob_versioned_hashes.len()` is rejected with `ReplacementShrinksBlobs { old_count, new_count }`. Without this, a sender could pin a blob slot with an N-blob transaction and then cheaply swap it for a 1-blob one, keeping the pool budget while dropping the data the network was told to expect. A non-blob transaction replacing a blob one is the degenerate case of the same shrink (N → 0) and is rejected identically.
...
| `ReplacementShrinksBlobs { old_count, new_count }` | RBF candidate carries fewer blobs than the in-pool tx (including a non-blob replacing a blob tx). |

...
- **Expiry-verifier frame gas is added back into the VERIFY budget.** The spec says such a frame "is ignored only when matching the four allowed prefix shapes; its gas limit still counts", but ethrex's `ValidationPrefix::frame_indices` omits expiry frames because that is exactly what shape matching needs. Pricing the prefix alone would let a transaction declare unbounded expiry-frame gas for free, so `verify_budget_cost` sums them separately. The EIP-8141 mempool budget keeps its existing behaviour.
...
- **Eligibility ignores the operator-tunable VERIFY budget.** `validate_prefix_structure` takes a `max_verify_gas` argument that `--mempool.max-verify-gas` controls; the eligibility path passes `u64::MAX` and checks `MAX_VERIFY_GAS_PER_TX` itself, so a node-local flag can never decide an attested verdict. This follows from eligibility and mempool admission being separate policies, a consequence the EIP states but does not draw out for clients sharing the code path.

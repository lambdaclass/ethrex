...
- `ethrex-vm` adds `ethrex-trie` as a dependency (for `GuestProgramState`
  which moves there). The heavy deps (`crossbeam`, `rayon`) that `ethrex-trie`
  pulls in already reach `ethrex-vm` and the guest program through other paths
  (`ethrex-common`, `ethrex-levm`), so no feature gating is needed.
...
|-------|----------|------------------|
| `ethrex-state-backend` | `StateReader`, `StateCommitter`, `Account`, `AccountMut`, `MerkleOutput`, `NodeUpdates`, `StateError` | Concrete backends, `Trie`, `Nibbles`, `Merkleizer` enum |
| `ethrex-trie` | `MptBackend`, `MptMerkleizer`, `Trie`, `Nibbles`, `Node`, `GuestProgramState`, `compute_*_root` (all five), `validate_block_body`, `genesis_block()` free function, `execution_witness_to_rpc` free function | `StateBackend` enum, `Store`, `Merkleizer` enum |
| `ethrex-storage` | `StateBackend` enum, `Merkleizer` enum, `Store`, layer cache, flat state generator | Trie internals |
| `ethrex-common` | `AccountUpdate`, `AccountState`, `AccountInfo`, `Code`, block types, `ExecutionWitness`, `RpcExecutionWitness` data structs | Trie types (`Trie`, `Nibbles`, `Node`, `TrieLogger`), `compute_*_root`, `validate_block_body` |
...
```rust
impl StateBackend {
    /// Create a streaming merkleizer for pipelined block execution.
    /// Workers are spawned immediately (MPT: 16 shard threads).
    ///
    /// Does NOT take &Store. Instead takes abstract openers so that
    /// ethrex-trie does not depend on ethrex-storage.
    pub fn new_merkleizer(
        &self,
        parent_state_root: H256,
        state_trie_opener: Box<dyn Fn(H256) -> Result<Trie, TrieError> + Send>,
        storage_trie_opener: Box<dyn StorageTrieOpener>,
    ) -> Result<Merkleizer, StateError> {
        match self {
            StateBackend::Mpt(_) => {
                Ok(Merkleizer::Mpt(MptMerkleizer::new(
                    parent_state_root,
                    state_trie_opener,
                    storage_trie_opener,
                )?))
...
let state_opener = store.make_state_trie_opener();
let storage_opener = store.make_storage_trie_opener(parent_state_root);
let merkleizer = state_backend.new_merkleizer(
    parent_state_root, state_opener, storage_opener,
)?;
...
Lives in `crates/common/trie/mpt_merkleizer.rs` (the `ethrex-trie` crate).
...
pub struct MptMerkleizer {
    ...
    worker_handles: Vec<JoinHandle<Result<(), StateError>>>,
    ...
    accumulated_updates: Vec<AccountUpdate>,
    // shutdown channels, watcher handle, drop sender for async BranchNode drop, etc.
}
...
impl MptMerkleizer {
    pub fn new(
        ...
        // Spawn 16 shard worker threads via crossbeam channels.
        // Each worker gets a clone of the storage_trie_opener (Arc-wrapped).
        // Workers use state_trie_opener to open their shard of the state trie.
    }

    pub fn feed_updates(&mut self, updates: Vec<AccountUpdate>) -> Result<(), StateError> {
        // Route each update to the correct shard by hashed_address[0] >> 4.
        // Collect code updates locally.
        // If accumulate_for_witness, clone updates into accumulated_updates.
        // This is the logic in handle_merkleization's `for updates in rx` loop.
    }

    pub fn finalize(self) -> Result<MerkleOutput, StateError> {
        // Send FinishRouting to all workers.
        // Collect sub-roots from all 16 workers.
        // Assemble final BranchNode root.
        // Convert (Nibbles, Vec<u8>) pairs to (Vec<u8>, Vec<u8>) for NodeUpdates.
        // Return MerkleOutput with NodeUpdates::Mpt.
    }
}
```

Types that move from `blockchain.rs` into `mpt_merkleizer.rs`:
- `WorkerRequest` enum
...
BAL workers get trie access through the same `StorageTrieOpener` abstraction.
...
### `UpdateBatch` migration

The current `UpdateBatch` struct (store.rs line ~274) uses
`Vec<TrieNode>` for `account_updates` and `Vec<(H256, Vec<TrieNode>)>` for
`storage_updates`, where `TrieNode = (Nibbles, NodeRLP)`. This is replaced
by `MerkleOutput`: the pipeline produces `MerkleOutput` from the merkleizer,
and `Store::store_block_updates` accepts `MerkleOutput` directly. The
`UpdateBatch` type is deleted.
...
| `types/genesis.rs` | `genesis_block()` method | **Move** to `ethrex-trie` as an extension `impl Genesis` block. It calls `compute_transactions_root` (in common, reachable from trie) and `compute_state_root` (in trie). |
| `types/block_execution_witness.rs` | `Node`, `NodeRef`, `Nibbles`, `EMPTY_TRIE_HASH`, `Trie` methods, `ExecutionWitness` trie logic | **Move** the trie-manipulation methods (`encode_subtrie`, witness reconstruction) to `ethrex-trie`. Keep `ExecutionWitness` and `RpcExecutionWitness` data structs in `ethrex-common` (they are referenced by `ethrex-storage`, `ethrex-blockchain`, and RPC). Add trie-specific extension methods via `impl ExecutionWitness` blocks in `ethrex-trie`. After moving, `EMPTY_TRIE_HASH` imports change to `crate::EMPTY_TRIE_HASH`. |
| `types/block_execution_witness.rs` | `TryFrom<ExecutionWitness> for RpcExecutionWitness` | **Replace** with a named free function in `ethrex-trie`: `pub fn execution_witness_to_rpc(w: ExecutionWitness) -> Result<RpcExecutionWitness, TrieError>`. Avoids the orphan rule problem (neither type is local to `ethrex-trie`). Callers switch from `.try_into()` to calling this function directly. |
...
| `vm/witness_db.rs` (or similar) | Imports `GuestProgramState` | **Update** to import `GuestProgramState` from `ethrex-trie`. Add `ethrex-trie` dep with `default-features = false` to `ethrex-vm`. |
| `guest-program/` | `GuestProgramState`, `MptBackend` usage | **Update** to import from `ethrex-trie`. Use `MptBackend::from_witness` for witness-based state construction. No `StateBackend` enum needed. |
...
- `ethrex-vm` depends on `ethrex-trie` (for `GuestProgramState`)
...
- `MptBackend` in `ethrex-trie` implementing `StateReader` + `StateCommitter`,
  with `from_witness` constructor
- `GuestProgramState` moved to `ethrex-trie`
- `compute_storage_root` and `compute_state_root` moved to `ethrex-trie`
- `genesis_block()` moved to `ethrex-trie` as free function (no extension traits)
- `execution_witness_to_rpc` free function in `ethrex-trie` (replaces `TryFrom`)
...
- [ ] `GuestProgramState` lives in `ethrex-trie`
- [ ] `MptBackend::from_witness` exists and builds tries from witness data
- [ ] `execution_witness_to_rpc` free function exists in `ethrex-trie`
- [ ] `compute_storage_root` and `compute_state_root` live in `ethrex-trie`
- [ ] `genesis_block()` lives in `ethrex-trie` as free function in `ethrex-trie`
...
| `WorkerRequest` enum (line ~261) | Delete. Moves to `mpt_merkleizer.rs`. |
| `CollectedStateMsg`, `PreMerkelizedAccountState`, `BalStateWorkItem` | Delete. Move to `mpt_merkleizer.rs`. |
| `collapse_root_node` helper (line ~2598) | Moves inside `MptMerkleizer`. |
| `DROP_SENDER` static (line ~127) | Moves to `mpt_merkleizer.rs` (async BranchNode drop). |
...
|-------------|--------|--------|
| `AccountUpdatesList` (line ~298) | Delete. Replaced by `MerkleOutput`. | TODO |
| `UpdateBatch` (line ~274) | Delete. `store_block_updates` accepts `MerkleOutput` directly. | TODO |
| `store_block` (line ~1727) | Accept `MerkleOutput` instead of `AccountUpdatesList`. | DONE |
| `apply_account_updates_batch` | Refactor to use `StateBackend` and return `MerkleOutput`. | TODO |
| `apply_account_updates_from_trie_batch` | Keep as internal MPT helper (`pub(crate)`). | TODO |
| `apply_account_updates_from_trie_with_witness` | Keep as internal MPT helper (`pub(crate)`). | TODO |
...
| `flatkeyvalue_generator` | Use `StateBackend::iter_accounts` / `iter_storage`. | TODO |
| `hash_address()`, `hash_key()` | Make `pub(crate)`. Add `mpt_hash_address`/`mpt_hash_key` to `ethrex-trie`. | TODO |
| `get_account_info_by_state_root` | Delegate to `new_state_reader().account()`. See Section 14. | TODO |
| `get_nonce_by_account_address` | Delegate to `new_state_reader().account()`. See Section 14. | TODO |
| `get_storage_at_root` | Delegate to `new_state_reader().storage()`. See Section 14. | TODO |
| `get_code_by_account_address` | Delegate through `StateReader`. See Section 14. | TODO |
| `get_storage_at_root_with_known_storage_root` | Delete. Use `MptBackend::storage_with_hint`. See Section 14. | TODO |
...
| `genesis.rs` `genesis_block()` | **Move** to `ethrex-trie` as free function. |
| `block_execution_witness.rs` trie logic | Move trie methods to `ethrex-trie` (as `impl ExecutionWitness` extension). Keep `ExecutionWitness` and `RpcExecutionWitness` data structs in `ethrex-common`. |
| `block_execution_witness.rs` `TryFrom<ExecutionWitness> for RpcExecutionWitness` | Replace with free function `execution_witness_to_rpc` in `ethrex-trie`. Callers switch from `.try_into()` to calling this function. |
...
| Current code | Action |
|-------------|--------|
| `db.rs` `VmDatabase` trait | No change. |
| `witness_db.rs` (or equivalent) imports of `GuestProgramState` | Update to import `GuestProgramState` from `ethrex-trie`. |
| `Cargo.toml` | Add `ethrex-trie` dep. |

### `crates/guest-program/`

| Current code | Action |
|-------------|--------|
| `GuestProgramState` usage | Import from `ethrex-trie`. |
| State construction from witness | Use `MptBackend::from_witness` instead of `GuestProgramState::from_witness`. |
| `Cargo.toml` | Add `ethrex-trie` dep. |
...
| `blockchain.rs`: witness generation imports `hash_address`, `hash_key` | Keccak for trie path derivation in witness code | Import from `ethrex-trie` as `mpt_hash_address`/`mpt_hash_key` (MPT-specific witness code matches on enum arm) |
...
impl Store {
    ...
    pub fn new_state_reader(&self, state_root: H256) -> Result<StateBackend, StoreError> {
        ...
        let storage_opener = self.make_storage_trie_opener(state_root);
...
`MptBackend` gains optional DB-backed accessors for on-demand storage trie
opening and code reading. These are `None` for the existing in-memory /
witness constructors (`new`, `from_witness`) and `Some` for the DB-backed
constructor.
...
pub struct MptBackend {
    ...
    /// On-demand storage trie opener (DB path). None for in-memory mode.
    storage_opener: Option<Arc<dyn StorageTrieOpener>>,
    /// On-demand code reader (DB path). None for in-memory mode.
    code_reader: Option<Arc<dyn Fn(H256) -> Result<Option<Vec<u8>>, TrieError> + Send + Sync>>,
}
```
...
| `hash_address()`, `hash_key()` | Make `pub(crate)` in store.rs. Add `mpt_hash_address`, `mpt_hash_key` to `ethrex-trie` for MPT-specific callers |
...
`hash_key` calls import from `ethrex_trie` (`mpt_hash_address`,
`mpt_hash_key`) instead of `ethrex_storage`. When binary trie is added in
...
- [ ] Witness code uses `mpt_hash_address`/`mpt_hash_key` from `ethrex-trie`

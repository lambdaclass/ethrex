...
### Pre-go-live checks (2026-09-17)

Six checks run before deploying `e5bc6086c` to the live chain, in the order listed; the
conformance script changes they prompted are in the same commit as this record.

| Check | Result |
| --- | --- |
| Live-chain replay | 40,303 blocks exported from the production node's local RPC (read-only, over ssh) and imported with the `e5bc6086c` binary against the published genesis: `Import completed blocks=40303 seconds=33.95`, no rejected block. Nothing since the live chain's commit changes how its history executes |
| Node restart | The ledger is in memory. After `docker restart` of el-1, a pool whose spends were 150 blocks back read width 0 while its peers read 4.9M and 3.8M: the first credit after a start covered only the finalized block itself, and later ones at most 64 back. A rolling upgrade would have put every relayer back to one pending transaction on each upgraded node until its next spend finalizes. Changed after the check: the first credit after a start covers the last 1,024 finalized blocks (3.4 hours at 12-second slots), so a restart keeps most earned width; forgotten spends over-credit by at most what was pending, under the cap |
| Pending lifetime | Devnet with a 30M gas limit and a 60-second lifetime. A 5M-gas target at a 1 wei tip was kept out of blocks by fillers burning 14.5M gas each (two per block, blocks 67 to 71, 29.0M of 32.2M used per block); at the head after the 60-second mark it was gone from the pool with no receipt while the fillers kept mining, and the pool then drained to empty. The sweep evicted the stale transaction and nothing else |
| 12-second slots | Devnet at production slot time. Pool lifecycle green; two-spend run fresh (A 94, C held then 95) and earned (both in 164, width 3,088,526 to 1,941,898); conformance complete across two runs including the paymaster section (three finalized sponsored transfers credited 659,091; pair admitted together, mined in block 512, paymaster charged to 257,447). Two script defects found and fixed: the finality wait was a fixed 15 minutes sized for 6-second slots and timed out while the chain finalized normally, now derived from the slot time (three epochs); the paymaster section's recipients were deterministic and a re-run on the same chain found them pre-existing, so the earning transfers paid no new-account gas and earned under one charge, now salted per run. The script also ran its suite on import; `main` is now guarded |
| Three nodes' ledgers | Sampled every three seconds through the two-spend run: credits identical everywhere (3,088,526 at finalized 95), pending counts identical, one momentary spend divergence of exactly one revalidation charge when node 3 imported a head first; all three converged once the pending set emptied |
| Faucet path | Both faucet-page submitters mined against the devnet: the self-paid EOA frame transfer, and the OpenSponsor-sponsored one twice in sequence; the non-canonical sponsor never appears in the ledger |

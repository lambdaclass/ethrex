...
| Node restart | The ledger is in memory. After `docker restart` of el-1, a pool whose spends were 150 blocks back read width 0 (catch-up credited blocks 254 to 317 only) while its peers read 4.9M and 3.8M. A rolling upgrade puts every relayer back to one pending transaction on each upgraded node until its next spend finalizes. The 64-block catch-up is one constant; 1,024 blocks would keep most earned width across a restart |

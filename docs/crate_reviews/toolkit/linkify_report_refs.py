...
def _resolve_repo_root(script_path: Path) -> Path:
    try:
        ...
        raise RuntimeError("Unexpected toolkit location; cannot locate repo root") from err
...
def _linkify(text: str, repo_root: Path) -> tuple[str, int]:
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        if tick == "" and start > 0 and text[start - 1] == "[" and end < len(text) and text[end] == "]":
            return match.group(0)
        path_part, line_part = token.rsplit(":", 1)
        full_path = _resolve_path(path_part, base_path, repo_root)
        if "-" in line_part:
            start_line, end_line = [segment.strip() for segment in line_part.split("-", 1)]
...
def _process_file(path: Path, repo_root: Path, dry_run: bool, verbose: bool) -> int:
    ...
    if verbose and replacements:
        ...
        print(f"{action} {path} ({replacements} link{'s' if replacements != 1 else ''})")
    return replacements
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verbose:
        ...
        print(f"{summary} {total} link{'s' if total != 1 else ''} across {len(args.targets)} target(s)")
    return 0

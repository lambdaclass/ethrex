...
| **Slot Time** | 6 seconds |
| **Fork** | Hegota (EIP-8141 frame-tx opcodes are gated on the Hegota fork) |
...
Each frame is a 6-tuple: `[mode, flags, target, gas_limit, value, data]` — `flags` bits 0-1 are the allowed APPROVE scope, bit 2 the atomic-batch flag; only SENDER frames may carry a non-zero `value`.

Each signature is `[scheme, signer, msg, signature]` — `scheme` 0 = secp256k1 (`signature` = `v‖r‖s`, 65 bytes, `v` ∈ {27,28}), 1 = P256; `msg` empty means the signature is over the canonical `sig_hash`.
...
`test-frame-tx.py` builds a **self-verified** frame transaction (Example 1) in the exact
current RLP encoding ethrex expects (6-field frames + `signatures` list) and confirms it
mines. No paymaster contracts required — the sender pays its own gas.

```bash
cd ethrex/scripts/eip8141-devnet

# Install dependencies (one-time)
python3 -m venv .venv && .venv/bin/pip install eth-keys "eth-hash[pycryptodome]"

# Self-verified frame tx (VERIFY self + SENDER transfer, sender pays gas)
.venv/bin/python3 test-frame-tx.py \
  --rpc-url https://rpc1.eip-8141.ethrex.xyz \
  --private-key <YOUR_FUNDED_KEY>
```

On success it prints the mined receipt with `"type": "0x6"`, `"status": "0x1"`, the recorded
`payer`, and per-frame `frameReceipts`.

> **Validation-prefix gas budget:** the sum of VERIFY-frame `gas_limit`s plus the signature
> verification cost (2800 per secp256k1 signature) must stay ≤ `MAX_VERIFY_GAS` (100,000) or
> the tx is rejected at admission — keep `--verify-gas` low (default 50,000).

**Paymaster-sponsored flows** (GasSponsor open / CanonicalPaymaster owner-signed, Examples 2-3)
require those contracts to be deployed on the target chain first via `deploy-contracts.sh`;
they are not deployed by default and the addresses above are reset on every devnet restart.

...
use ethrex_l2_sdk::{
    L1ToL2TransactionData, L2_WITHDRAW_SIGNATURE, build_generic_tx, get_last_verified_batch,
    send_generic_transaction, wait_for_l1_message_proof, wait_for_l2_deposit_receipt,
};
...
async fn l2_integration_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Consume the two private keys that test_fee_token would have used.
    let _unused_fee_token_key1 = private_keys.pop().unwrap();
    let _unused_fee_token_key2 = private_keys.pop().unwrap();

...
async fn parallel_path_rejects_invalid_block_access_list_hash() {
    ...
    let valid = bc_ok.add_block_pipeline_bal(block.clone(), Some(Arc::new(bal.clone())));
    assert!(
        valid.is_ok(),
    ...
    let par = bc_par.add_block_pipeline_bal(block.clone(), Some(Arc::new(bal.clone())));
    ...
    let seq = bc_seq.add_block_pipeline_bal(block.clone(), Some(Arc::new(bal.clone())));

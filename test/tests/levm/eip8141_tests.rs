...
mod intrinsic_gas_accounting_tests {
    ...
    fn intrinsic_gas_is_insensitive_to_structural_frame_fields() {
        ...
        // frame gas: the frame gas limits are a term of `standard_gas_limit`, so
        ...
        // A frame gas limit is the one structural field `standard_gas_limit` tracks,
        // and it passes through with no other term moving.
        assert_eq!(
            restructured.standard_gas_limit(),
            baseline.standard_gas_limit() + 1_000
        );
    }
}

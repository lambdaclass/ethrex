...
fn test_decode_bool() {
    let rlp = vec![0x01];
    let decoded = bool::decode(&mut rlp.as_slice()).unwrap();
    assert!(decoded);
...
fn test_decode_u8() {
    let rlp = vec![0x01];
    let decoded = u8::decode(&mut rlp.as_slice()).unwrap();
    assert_eq!(decoded, 1);
...
fn test_decode_u32() {
    ...
    let decoded = u32::decode(&mut rlp.as_slice()).unwrap();
...
fn test_decode_fixed_length_array() {
    let rlp = vec![0x0f];
    let decoded = <[u8; 1]>::decode(&mut rlp.as_slice()).unwrap();
    assert_eq!(decoded, [0x0f]);

    let rlp = vec![RLP_NULL + 3, 0x02, 0x03, 0x04];
    let decoded = <[u8; 3]>::decode(&mut rlp.as_slice()).unwrap();
...
fn test_decode_ip_addresses() {
    ...
    let decoded = Ipv4Addr::decode(&mut rlp.as_slice()).unwrap();
    ...
    let decoded = Ipv6Addr::decode(&mut rlp.as_slice()).unwrap();
...
#[test]
fn test_decode_u256() {
    // Single byte values < 0x80 must use canonical encoding (just the byte itself)
    let rlp = vec![0x01];
    let decoded = U256::decode(&mut rlp.as_slice()).unwrap();
...
fn test_decode_string() {
    ...
    let decoded = String::decode(&mut rlp.as_slice()).unwrap();
...
fn test_decode_lists() {
    ...
    let decoded: Vec<String> = decode_list(&mut rlp.as_slice()).unwrap();
    let expected: Vec<String> = vec![];
    assert_eq!(decoded, expected);

    //  list with a single number
    let rlp = vec![RLP_EMPTY_LIST + 1, 0x01];
    let decoded: Vec<u8> = decode_list(&mut rlp.as_slice()).unwrap();
    let expected = vec![1];
    assert_eq!(decoded, expected);
...
fn test_decode_list_of_lists() {
    ...
    // In librlp, Vec<u8> decodes as bytes, not a list. For list-of-lists,
    // we manually parse the outer list header and decode_list each inner list.
    let mut buf = rlp.as_slice();
    let outer = librlp::Header::decode(&mut buf).unwrap();
    assert!(outer.list);
    let mut payload = &buf[..outer.payload_length];
    let mut decoded: Vec<Vec<u8>> = Vec::new();
    while !payload.is_empty() {
        decoded.push(decode_list(&mut payload).unwrap());
    }
    ...
    assert!(outer.list);
    let mut payload = &buf[..outer.payload_length];
    let mut decoded: Vec<Vec<String>> = Vec::new();
    while !payload.is_empty() {
        decoded.push(decode_list(&mut payload).unwrap());
    }
    let expected = vec![
...
fn test_decode_tuples() {
    ...
    let decoded: (u8, u8) = <(u8, u8)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (1, 2);
    assert_eq!(decoded, expected);

    // tuple with string and number
    let rlp = vec![RLP_EMPTY_LIST + 5, 0x01, 0x83, b'c', b'a', b't'];
    let decoded: (u8, String) = <(u8, String)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (1, "cat".to_string());
    assert_eq!(decoded, expected);

    // tuple with bool and string
    let rlp = vec![RLP_EMPTY_LIST + 6, 0x01, 0x84, b't', b'r', b'u', b'e'];
    let decoded: (bool, String) = <(bool, String)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (true, "true".to_string());
    assert_eq!(decoded, expected);

    // tuple with bytes and number (Vec<u8> is bytes, not a list)
    let rlp = vec![RLP_EMPTY_LIST + 2, RLP_NULL, 0x03];
    let decoded = <(Vec<u8>, u8)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (vec![], 3);
    assert_eq!(decoded, expected);

    // tuple with number and bytes
    let rlp = vec![RLP_EMPTY_LIST + 2, 0x03, RLP_NULL];
    let decoded = <(u8, Vec<u8>)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (3, vec![]);
    assert_eq!(decoded, expected);
...
fn test_decode_tuples_3_elements() {
    // tuple with numbers
    let rlp = vec![RLP_EMPTY_LIST + 3, 0x01, 0x02, 0x03];
    let decoded: (u8, u8, u8) = <(u8, u8, u8)>::decode(&mut rlp.as_slice()).unwrap();
    let expected = (1, 2, 3);
    assert_eq!(decoded, expected);

    // tuple with string and number
    let rlp = vec![RLP_EMPTY_LIST + 6, 0x01, 0x02, 0x83, b'c', b'a', b't'];
    let decoded: (u8, u8, String) = <(u8, u8, String)>::decode(&mut rlp.as_slice()).unwrap();
...
fn test_decode_list_as_string() {
    ...
    let decoded: Result<[u8; 4], _> = RlpDecode::decode(&mut rlp.as_slice());
    // It should fail because a list is not a string
    assert!(decoded.is_err());

    // [1, 2] != 0x0102
    let rlp = vec![RLP_EMPTY_LIST + 2, 0x01, 0x02];
    let decoded: Result<u16, _> = RlpDecode::decode(&mut rlp.as_slice());
    // It should fail because a list is not a string
    assert!(decoded.is_err());
}

...
async fn trace_tx_mux_tracer() {
    ...
    assert!(
        obj.contains_key("callTracer"),
        "should have callTracer result"
    );
    assert!(
        obj.contains_key("prestateTracer"),
        "should have prestateTracer result"
    );

    // callTracer sub-result should be a CALL frame.
    let call_result = obj["callTracer"]
        .as_object()
        .expect("callTracer result should be object");
    assert_eq!(call_result["type"].as_str().unwrap(), "CALL");

    // prestateTracer sub-result should be an address map.
    let prestate_result = obj["prestateTracer"]
        .as_object()
        .expect("prestateTracer result should be object");
    assert!(
        !prestate_result.is_empty(),
        "prestateTracer result should not be empty"
    );
}

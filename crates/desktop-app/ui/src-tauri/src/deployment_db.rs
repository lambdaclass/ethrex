    ...
    let rows = stmt
        .query_map([], |row| {
            Ok(DeploymentRow {
                ...
                rpc_url: row.get(5)?,
                status: row.get(6)?,
                deploy_method: row.get(7)?,
                docker_project: row.get(8)?,
                l1_port: row.get(9)?,
                l2_port: row.get(10)?,
                proof_coord_port: row.get(11)?,
                phase: row.get(12)?,
                bridge_address: row.get(13)?,
                proposer_address: row.get(14)?,
                timelock_address: row.get(15)?,
                sp1_verifier_address: row.get(16)?,
                guest_program_registry_address: row.get(17)?,
                verification_status: row.get(18)?,
                error_message: row.get(19)?,
                config: row.get(20)?,
                is_public: row.get(21)?,
                created_at: row.get(22)?,
                tools_l1_explorer_port: row.get(23)?,
                tools_l2_explorer_port: row.get(24)?,
                tools_bridge_ui_port: row.get(25)?,
                hashtags: row.get(26)?,
                ever_running: row.get::<_, Option<i64>>(27)?.unwrap_or(0),
            })
        })

...
impl ExecBackend {
    ...
    ///
    /// On L1, dispatches at runtime on the block's fork: Hegotá-active blocks
    /// run the canonical EIP-8025 path; pre-Hegotá blocks run the legacy path.
    /// On L2, always runs the L2 legacy path (Hegotá / EIP-8025 do not apply).
    fn execute_core(input: ProgramInput) -> Result<ProgramOutput, BackendError> {
        let crypto = Arc::new(NativeCrypto);

        #[cfg(not(feature = "l2"))]
        {
            use ethrex_guest_program::l1::Eip8025ProgramOutput;

            let chain_config = input.execution_witness.chain_config;
            let first_block_timestamp = input
                .blocks
                .first()
                .map(|b| b.header.timestamp)
                .unwrap_or(0);

            if chain_config.is_hegota_activated(first_block_timestamp) {
                ...
                // The zeroed new_payload_request_root is intentional — the
                // authoritative root is tracked by the coordinator's ProofRequest
                // and checked there.
                Ok(ProgramOutput::Eip8025(Eip8025ProgramOutput {

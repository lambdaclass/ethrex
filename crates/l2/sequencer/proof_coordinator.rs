...
impl ProofCoordinator {
    ...
    async fn create_prover_input(
        ...
        let Ok(witness) = self.blockchain.generate_witness_for_blocks(&blocks).await else {
        ...
        let chain_config = self.store.get_chain_config().map_err(|_| {
            ProverServerError::InternalError("Failed to get chain config".to_string())
        })?;

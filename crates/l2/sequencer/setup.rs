...
use ethrex_common::{Bytes, Address};
...
pub async fn register_tdx_key(eth_client: &EthClient, private_key: &SecretKey, on_chain_proposer_address: Address, quote: Bytes) -> Result<(), ProverServerError> {
...
async fn get_tdx_address(eth_client: &EthClient, on_chain_proposer_address: Address) -> Result<Address, ProverServerError> {
    ...
    Address::from_str(&format!("0x{trimmed_response}")).map_err(|_| {
        ProverServerError::InternalError("Failed to convert TDXVERIFIER result to address".to_owned())
    })
}

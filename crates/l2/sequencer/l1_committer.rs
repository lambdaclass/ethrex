...
impl L1Committer {
        ...
        let batch = match self.rollup_store.get_batch(batch_to_commit).await? {
            ...
            None => {
                let last_committed_blocks = self
                    ...
                    .await?
                    .ok_or(
                        CommitterError::OtherError(format!("Failed to get batch with batch number {last_committed_batch_number}. Batch is missing when it should be present. This is a bug"))
                    )?;
                let last_block = last_committed_blocks
                    .last()
                    .ok_or(
                        CommitterError::OtherError(format!("Last committed batch ({last_committed_batch_number}) doesn't have any blocks. This is probably a bug."))
                    )?;
        ...
        loop {
            ...
            for (index, tx) in block_to_commit_body.transactions.iter().enumerate() {
                let receipt = self
                    ...
                    .await?
                    .ok_or(CommitterError::OtherError(
    ...
    async fn send_commitment(&mut self, batch: &Batch) -> Result<H256, CommitterError> {
        ...
        let gas_price = self
            ...
            .try_into()
            .map_err(|_| {
                CommitterError::OtherError("Failed to convert gas_price to a u64".to_owned())
            })?;

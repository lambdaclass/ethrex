        ...
        if let Some(cb) = &circuit_breaker {
            if !head_tx.is_privileged() {
                ...
                let tx_execution_id = Some(TxExecutionId {
                    block_number: block_num_bytes,
                    iteration_id: cb.current_iteration_id(),
                    tx_hash: tx_hash.as_bytes().to_vec(),
                    index: tx_index,
                });
                let sender = head_tx.tx.sender();
                let tx_as_inner: Transaction = head_tx.clone().into();
                let tx_env = build_transaction_env(&tx_as_inner, sender);
                let sidecar_tx = SidecarTransaction {
                    tx_execution_id,
                    tx_env: Some(tx_env),
                    prev_tx_hash: None,
                };
                if !cb.evaluate_transaction(sidecar_tx).await {
                    debug!("Circuit breaker rejected transaction: {tx_hash:#x}");
                    txs.pop();
...
fn build_transaction_env(tx: &Transaction, sender: ethrex_common::Address) -> TransactionEnv {
    ...
    // gas_price fits in u128 for all practical purposes

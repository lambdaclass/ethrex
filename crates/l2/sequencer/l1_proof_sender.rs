...
impl L1ProofSender {
    async fn new(
        ...
        let l1_chain_id = eth_client.get_chain_id().await?.try_into().map_err(|_| {
            ProofSenderError::UnknownError("Failed to convert chain ID to U256".to_owned())
        })?;
    ...
    async fn verify_and_send_proof(&mut self) -> Result<(), ProofSenderError> {
        ...
        .map_err(|err| {
            ...
            ProofSenderError::UnknownError(err.to_string())
        })?;
    ...
    async fn send_proof_to_aligned(
        ...
        let elf = std::fs::read(self.aligned_sp1_elf_path.clone())
            .map_err(|e| ProofSenderError::UnknownError(format!("Failed to read ELF file: {e}")))?;
        ...
        let Signer::Local(local_signer) = &self.signer else {
            return Err(ProofSenderError::UnknownError(
                "Aligned mode only supports local signer".to_string(),
            ));
        };

        let wallet = Wallet::from_bytes(local_signer.private_key.as_ref())
            .map_err(|_| ProofSenderError::UnknownError("Failed to create wallet".to_owned()))?;

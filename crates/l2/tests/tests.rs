...
const DEFAULT_L1_RICH_WALLET_ADDRESS: Address = H160([
    0x44, 0x17, 0x09, 0x2b, 0x70, 0xa3, 0xe5, 0xf1, 0x0d, 0xc5, 0x04, 0xd0, 0x94, 0x7d, 0xd2, 0x56,
    0xb9, 0x65, 0xfc, 0x62,
]);
...
async fn l2_deposit_with_contract_call() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        hex::encode(calldata_to_contract.as_ref())
    );
    ...
    let values = vec![Value::Tuple(vec![
        ...
        Value::Bytes(calldata_to_contract.clone()), // data
    ])];

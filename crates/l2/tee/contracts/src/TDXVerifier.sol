...
contract TDXVerifier {
    ...
    ISettlement public Settlement = ISettlement(address(0));
    ...
    /// @param _ocp Settlement contract, used for permission checks
    ...
    constructor(address _dcap, address _ocp, bool _isDevMode) {
        ...
        require(
            _ocp != address(0),
            "TDXVerifier: OnChainPropser address can't be null"
        );

        quoteVerifier = IAttestation(_dcap);
        Settlement = ISettlement(_ocp);
    ...
    function register(bytes calldata quote) external {
        require(
            Settlement.authorizedSequencerAddresses(msg.sender),

...
async fn handle_getkey(State(state): State<Arc<AppState>>) -> Result<Json<GetKeyResponse>, StatusCode> {
    let mut digest_slice = [0u8; 64];
    digest_slice.split_at_mut(20).0.copy_from_slice(state.signer.address().as_bytes());
...
async fn handle_transition(
    ...
    let signature = state.signer.sign_message(&bound_data).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

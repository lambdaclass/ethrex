...
use ethrex_l2_common::{
    ...
    messages::{
        L1MessageProof, L2MessageProof, get_block_l1_messages, get_block_l2_messages,
...
pub struct GetL2MessageProof {
    pub transaction_hash: H256,
}

impl RpcHandler for GetL2MessageProof {
    fn parse(params: &Option<Vec<Value>>) -> Result<GetL2MessageProof, RpcErr> {
        let params = params.as_ref().ok_or(ethrex_rpc::RpcErr::BadParams(
            "No params provided".to_owned(),
        ))?;
        if params.len() != 1 {
    ...
    async fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        // Gets the message hashes from the transaction
        let tx_messages = get_block_l2_messages(&[tx_receipt]);
        let tx_messages_by_hash = tx_messages
            .iter()
            .map(|msg| (get_l2_message_hash(msg), msg))
            .collect::<HashMap<_, _>>();
        ...
        let mut proofs = vec![];
        for (index, message_hash) in batch_message_hashes.iter().enumerate() {
            if !tx_messages_by_hash.contains_key(message_hash) {
                continue;
            }

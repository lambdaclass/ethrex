...
fn build_risc0_program() {
    ...
    let _ = std::fs::create_dir("../../ethrex-guest/bin/risc0/out");

    std::fs::write("../../ethrex-guest/bin/risc0/out/riscv32im-risc0-elf", &elf)
...
fn build_sp1_program() {
    ...
    sp1_build::build_program_with_args(
        "../../ethrex-guest/bin/sp1",
        sp1_build::BuildArgs {
            output_directory: Some("../../ethrex-guest/bin/sp1/out".to_string()),
    ...
    let elf = std::fs::read("../../ethrex-guest/bin/sp1/out/riscv32im-succinct-zkvm-elf")
        .expect("could not read SP1 elf file");
    let prover = ProverClient::from_env();
    let (_, vk) = prover.setup(&elf);

    std::fs::write(
        "../../ethrex-guest/bin/sp1/out/riscv32im-succinct-zkvm-vk-bn254",
        format!("{}\n", vk.vk.bytes32()),
    )
    .expect("could not write SP1 vk-bn254 to file");
    std::fs::write(
        "../../ethrex-guest/bin/sp1/out/riscv32im-succinct-zkvm-vk-u32",
...
fn build_zisk_program() {
    ...
    build_command
        ...
        .current_dir("../../ethrex-guest/bin/zisk");
    #[cfg(not(feature = "ci"))]
    {
        setup_command
            .env("RUSTC", rustc_path("zisk"))
    ...
    let _ = std::fs::create_dir("../../ethrex-guest/bin/zisk/out");

    std::fs::copy(
        "../../ethrex-guest/bin/zisk/target/riscv64ima-zisk-zkvm-elf/release/ethrex-guest-zisk",
        "../../ethrex-guest/bin/zisk/out/riscv64ima-zisk-elf",
    )
...
fn build_openvm_program() {
    ...
    let status = Command::new("cargo")
        ...
        .current_dir("../../ethrex-guest/bin/openvm")
        .status()
    ...
    let elf_src = Path::new(
        "../../ethrex-guest/bin/openvm/target/riscv32im-risc0-zkvm-elf/release/ethrex-guest-openvm",
    );
    let elf_dst = Path::new("../../ethrex-guest/bin/openvm/out/riscv32im-openvm-elf");

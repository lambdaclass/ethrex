...
const L2_UPGRADEABLE_RUNTIME: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/contracts/solc_out/L2Upgradeable.bytecode"
));
...
fn add_with_proxy(
    ...
    let proxy_code = L2_UPGRADEABLE_RUNTIME.to_vec();
...
fn update_genesis_file(l2_genesis_path: &PathBuf) -> Result<(), SystemContractsUpdaterError> {
    ...
    let l2_bridge_runtime = COMMON_BRIDGE_L2_RUNTIME.to_vec();
    add_with_proxy(&mut genesis, COMMON_BRIDGE_L2_ADDRESS, l2_bridge_runtime)?;

    let l1_messenger_runtime = L2_TO_L1_MESSENGER_RUNTIME.to_vec();
    add_with_proxy(
        &mut genesis,

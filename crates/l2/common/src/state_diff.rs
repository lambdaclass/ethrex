...
impl StateDiff {
    ...
    pub fn is_equivalent_to_account_updates(
        &self,
        account_updates: &HashMap<Address, AccountUpdate>,
    ) -> bool {
        for (address, diff) in &self.modified_accounts {
            let Some(update) = account_updates.get(address) else {
                return false;
            };
            let AccountUpdate {
                info: update_info,
                code: update_code,
                added_storage: update_storage,
                ..
            } = update;

            let AccountStateDiff {
                new_balance: diff_balance,
                nonce_diff: _diff_nonce, // TODO: nonce
                storage: diff_storage,
                bytecode: diff_code,
                bytecode_hash: diff_code_hash,
            } = diff;

            // TODO: nonce
            let diff_info = if let (Some(code_hash), Some(balance)) = (diff_code_hash, diff_balance)
            {
                Some(AccountInfo {
                    code_hash: *code_hash,
                    balance: *balance,
            ...
            // TODO: nonce
            // Check account info
            match (update_info, diff_info) {
                (Some(update_info), Some(diff_info)) => {
                    if diff_info.balance != update_info.balance
                        || diff_info.code_hash != update_info.code_hash
                    {
                        return false;
                    }
                }
            ...
            // Check code
            match (update_code, diff_code) {
                (Some(update_code), Some(diff_code)) => {
            ...
            // Check storage
            for (key, update_value) in update_storage {
                let Some(diff_value) = diff_storage.get(key) else {
                    return false;
                };
                if update_value != diff_value {
                    return false;
                }
            }
        }

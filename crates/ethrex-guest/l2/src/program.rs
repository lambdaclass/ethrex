...
use crate::blobs::verify_blob;
use crate::error::L2ExecutionError;
use crate::input::ProgramInput;
use crate::messages::{compute_message_digests, get_batch_messages};
...
pub fn execution_program(input: ProgramInput) -> Result<ProgramOutput, L2ExecutionError> {
    ...
    let BlockExecutionResult {
        ...
        execution_witness,
        elasticity_multiplier,
        |db: &GuestProgramStateWrapper, i: usize| -> Result<Evm, ExecutionError> {
            // L2 VM factory - requires fee config for each block
            let fee_config = fee_configs.get(i).cloned().ok_or_else(|| {
                ExecutionError::Internal("FeeConfig not provided for L2 execution".to_string())
            })?;
            Evm::new_for_l2(db.clone(), fee_config).map_err(ExecutionError::Evm)
        },
    )?;

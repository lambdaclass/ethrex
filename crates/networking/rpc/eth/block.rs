...
impl RpcHandler for GetBlockByNumberRequest {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let header = context.chain.store().get_block_header(block_number)?;
        let body = context.chain.store().get_block_body(block_number)?;
        ...
        let total_difficulty = context.chain.store().get_block_total_difficulty(hash)?;
...
impl RpcHandler for GetBlockReceiptsRequest {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let receipts =
            get_all_block_rpc_receipts(block_number, header, body, context.chain.store())?;
...
impl RpcHandler for GetRawReceipts {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let receipts: Vec<String> =
            get_all_block_receipts(block_number, header, body, context.chain.store())?
                .iter()
...
impl RpcHandler for BlockNumberRequest {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        serde_json::to_value(format!(
            "{:#x}",
            context.chain.store().get_latest_block_number()?
        ))
        .map_err(|error| RpcErr::Internal(error.to_string()))
    }
}
...
impl RpcHandler for GetBlobBaseFee {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let block_number = context.chain.store().get_latest_block_number()?;
        ...
        let parent_header = match context.chain.find_parent_header(&header) {
            Ok(header) => header,

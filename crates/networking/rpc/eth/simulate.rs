...
mod tests {
    ...
    // ── Integration tests (ported from execution-apis test vectors) ───────────

    /// Helper: create an RpcApiContext with in-memory storage and genesis state.
    async fn test_context() -> RpcApiContext {
        let store = crate::test_utils::setup_store().await;
        crate::test_utils::default_context_with_storage(store).await
    }

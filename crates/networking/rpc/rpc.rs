...
pub async fn handle_http_request(
    ...
    let res = match req {
        RpcRequestWrapper::Single(request) => {
            let res = map_http_requests(&request, service_context);
        ...
        RpcRequestWrapper::Multiple(requests) => {
            let responses: Vec<Value> = requests
                .into_iter()
                .map(|request| {
                    let res = map_http_requests(&request, service_context.clone());
                    rpc_response(request.id, res)
                })
                .collect();
...
mod tests {
    ...
    fn map_http_requests_single_item_then_array_json_test() {
        ...
        // Process request with a single value
        let body = r#"{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}"#;
        let expected_response = format!(r#"{{"id":67,"jsonrpc": "2.0","result": "{}"}}"#, chain_id);
        let expected_response = to_rpc_response_success_value(&expected_response);
        let req = serde_json::from_str::<RpcRequestWrapper>(body).unwrap();
        // serde should deserialize into RpcRequestWrapper::Single
        let res = match req {
            RpcRequestWrapper::Single(request) => {
                let res = map_http_requests(&request, context.clone());
                rpc_response(request.id, res)
            }
            RpcRequestWrapper::Multiple(_) => {
                panic!("Serde deserialized as RpcRequestWrapper::Multiple when it should be RpcRequestWrapper::Single")
            }
        };
        assert_eq!(res.to_string(), expected_response.to_string());
        // Process request with array request
        let body = r#"[{"jsonrpc":"2.0","method":"net_version","params":[],"id":67},{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}]"#;
        let expected_response =
            serde_json::to_value([expected_response.clone(), expected_response.clone()]).unwrap();
        let req = serde_json::from_str::<RpcRequestWrapper>(body).unwrap();
        // serde should deserialize into RpcRequestWrapper::Multiple
        let res = match req {
            RpcRequestWrapper::Multiple(requests) => {
                let responses: Vec<Value> = requests
                    .into_iter()
            ...
            RpcRequestWrapper::Single(_) => {
                panic!("Serde deserialized as RpcRequestWrapper::Single when it should be RpcRequestWrapper::Multiple")
            }
        };
        assert_eq!(res.to_string(), expected_response.to_string());
    }

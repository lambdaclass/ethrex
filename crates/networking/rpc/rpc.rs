...
pub async fn map_engine_requests(
    ...
    match req.method.as_str() {
        ...
        unknown_engine_method => {
            // EIP-8025 proof endpoints (behind feature flag)
            #[cfg(feature = "eip-8025")]
            match unknown_engine_method {
                "engine_requestProofsV1" => {
                    return RequestProofsV1::call(req, context).await;
                }
                "engine_verifyExecutionProofV1" => {
                    return VerifyExecutionProofV1::call(req, context).await;
                }
                "engine_verifyNewPayloadRequestHeaderV1" => {
                    return VerifyNewPayloadRequestHeaderV1::call(req, context).await;
                }
                _ => {}
            }
            Err(RpcErr::MethodNotFound(unknown_engine_method.to_owned()))
        }
    }
}

...
fn compute_new_payload_request_root(
    ...
    let ssz_header = stateless_ssz::NewPayloadRequestHeader {
        execution_payload_header: stateless_ssz::ExecutionPayloadHeader {
            parent_hash: [0u8; 32], // Placeholder -- actual computation in ProofEngine
            fee_recipient: stateless_ssz::Bytes20([0u8; 20]),
            state_root: [0u8; 32],
...
fn json_header_to_ssz_root(header: &EngineNewPayloadRequestHeaderV1) -> Result<[u8; 32], String> {
    ...
    let ssz_logs_bloom: stateless_ssz::LogsBloom = bloom_bytes
        .try_into()
        .map_err(|_| "logs_bloom conversion failed".to_string())?;
    ...
    let ssz_header = stateless_ssz::NewPayloadRequestHeader {
        execution_payload_header: stateless_ssz::ExecutionPayloadHeader {
            ...
            fee_recipient: stateless_ssz::Bytes20(ep.fee_recipient.0),

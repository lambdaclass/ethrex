...
impl ExecutionPayload {
    /// Converts an `ExecutionPayload` reference into a block without consuming self.
    /// Only clones the small fields (extra_data, withdrawals), while decoding
    /// transactions in parallel from references - avoiding the full payload clone.
    pub fn to_block(
        &self,
        ...
        let body = BlockBody {
            // Decode transactions in parallel using rayon - no clone needed
            transactions: self
                .transactions
                .par_iter()
    ...
    pub fn into_block(
        ...
        let body = BlockBody {
            // Decode transactions in parallel using rayon
            transactions: self
                .transactions
                .par_iter()
                .map(|encoded_tx| encoded_tx.decode())

...
impl ExecutionPayload {
    ...
    /// Takes `&self` so callers don't have to clone the full payload — the
    /// only fields actually moved are the transactions (decoded per-element)
    /// and the small `withdrawals`/`extra_data` (cheap clone / refcount bump).
    pub fn to_block(
        &self,

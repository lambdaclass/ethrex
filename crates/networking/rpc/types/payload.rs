...
mod test {
    ...
    fn deserialize_payload_into_block() {
        ...
        // Reference: decode the transactions sequentially, in order.
        let sequential: Vec<Vec<u8>> = payload
            .transactions
            .iter()
        ...
        let parallel: Vec<Vec<u8>> = block
            ...
            .transactions
            .iter()
            .map(|t| t.encode_canonical_to_vec())
            .collect();
        assert_eq!(
            parallel, sequential,
            "parallel tx decode must preserve order and content"
        );
    }

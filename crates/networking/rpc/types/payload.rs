...
pub struct PayloadStatus {
    ...
    /// payload satisfied its inclusion list. Carries a value only when the
    /// payload is `VALID`; an unsatisfied inclusion list leaves the payload
    /// `VALID` with `Some(false)`, so the consensus layer knows not to attest
    /// to it. Absent for every pre-Bogotá method, whose responses are
    /// `PayloadStatusV1`.
...
impl PayloadStatus {
    ...
    /// Records the EIP-7805 (FOCIL) inclusion-list verdict on a `VALID` payload
    /// status. Per execution-apis `PayloadStatusV2`, an unsatisfied inclusion
    /// list does not change the status — the payload stays `VALID` and only
    /// `inclusionListSatisfied` reports the verdict.

...
impl SyncManager {
    ...
    async fn sync_cycle(
        ...
        loop {
            ...
            let peer = get_peer_channel_and_wait(self.peers.clone(), Capability::Eth).await;
...
async fn download_and_run_blocks(
    ...
    loop {
        let peer = get_peer_channel_and_wait(peers.clone(), Capability::Eth).await;
...
async fn rebuild_state_trie(
    ...
    while retry_count <= MAX_RETRIES {
        let peer = get_peer_channel_and_wait(peers.clone(), Capability::Snap).await;
...
async fn get_peer_channel_and_wait(

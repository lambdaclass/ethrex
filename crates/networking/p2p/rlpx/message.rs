    ...
    pub fn decode(
        ...
        if msg_id < eth_version.eth_capability_offset() {
            ...
            match msg_id - eth_version.eth_capability_offset() {
                ...
                // eth/71 (EIP-8159) and eth/72 (EIP-8070) build on eth/69 — they use the
                // same receipt format as eth/69.
                GetReceipts69::CODE
                    if matches!(
                        eth_version,
                        EthCapVersion::V69 | EthCapVersion::V71 | EthCapVersion::V72
                    ) =>
                {
...
impl Display for Message {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Message::GetReceipts69(_) => "eth:GetReceipts(69)".fmt(f),

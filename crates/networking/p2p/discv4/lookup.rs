    ...
    async fn start_lookup_loop(&self, initial_interval_wait_seconds: u64) {
        ...
        loop {
            ...
            self.ctx.tracker.spawn({
                ...
                async move {
                    self_clone
                        .recursive_lookup(self_clone.ctx.local_node.public_key)
                        .await
                }
            });
            ...
            for _ in 0..3 {
                ...
                self.ctx.tracker.spawn({
                    ...
                    async move {
                        self_clone
                            .recursive_lookup(public_key_from_signing_key(&random_pub_key))
                            .await
                    }
                });
            }
...
mod tests {
    ...
    async fn discovery_server_lookup() -> Result<(), DiscoveryError> {
        let mut server_a = start_discovery_server(8000, Some(1), true).await?;
        let mut server_b = start_discovery_server(8001, Some(1), true).await?;
    ...
    async fn discovery_server_recursive_lookup() -> Result<(), DiscoveryError> {
        let mut server_a = start_discovery_server(8002, Some(1), true).await?;
        let mut server_b = start_discovery_server(8003, Some(1), true).await?;
        let mut server_c = start_discovery_server(8004, Some(1), true).await?;
        let mut server_d = start_discovery_server(8005, Some(1), true).await?;
        ...
        lookup_handler
            .recursive_lookup(server_a.ctx.local_node.public_key)
            .await;

...
impl DiscoveryServer {
    ...
    async fn handle_who_are_you(&mut self, packet: Packet) -> Result<(), DiscoveryServerError> {
        ...
        tracing::trace!("Built challenge_data: {challenge_data:?}");
        ...
        let ephemeral_pubkey = &ephemeral_key
        ...
        tracing::trace!("Obtained public key: {dest_pubkey}");
        ...
        tracing::trace!("derived session keys: {session:?}");
    ...
    async fn send_handshake(
        ...
        let packet = DecodedPacket::Handshake(Handshake {
            src_id: self.local_node.node_id(),
        ...
        trace!(msg = %message, node = %node.public_key, address= %addr, nonce=?nonce, key=?encrypt_key, record=?self.local_node_record, "Discv5 handshake message sent");
        self.messages_by_nonce
            .insert(nonce, (node.clone(), message.clone(), Instant::now()));
        Ok(())
    }

...
impl DownloadCoordinatorState {
    ...
    async fn prepare_download_tasks(&mut self, sync_head_hash: H256) {
    ...
    async fn refresh_downloaders(&mut self) {
    ...
    async fn assign_download_tasks(&mut self, self_handle: GenServerHandle<DownloadCoordinator>) {
        let mut downloaders = self.downloaders.lock().await;
...
impl GenServer for DownloadCoordinator {
    ...
    async fn handle_call(
        ...
        match message {
            CallMessage::DownloadHeaders { sync_head_hash } => {
                info!("Downloading headers up to sync head: {sync_head_hash}");

                loop {
                    state.prepare_download_tasks(sync_head_hash).await;

                    state.assign_download_tasks(handle.clone()).await;

                    state.refresh_downloaders().await;
                }

                CallResponse::Reply(
                    state,
                    Self::OutMsg::BlockHeaders(state.downloaded_headers.lock().await.clone()),
                )
            }
        }
    }

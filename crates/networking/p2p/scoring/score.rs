...
impl PeerScoringConfig {
    ...
    pub fn validate(&self) -> bool {
        let sum =
            self.reliability_weight + self.latency_weight + self.throughput_weight + self.compliance_weight;
        (sum - 1.0).abs() < 0.01
    }
}
...
impl PeerScore {
    ...
    pub fn record_success(
        ...
        let metrics = self
            .request_metrics
            .entry(request_type)
            .or_insert_with(RequestTypeMetrics::new);
    ...
    fn compute_latency_score(&self, metrics: &RequestTypeMetrics) -> f64 {
        ...
        let score = 1.0 - (ewma_ms - LATENCY_REFERENCE_MS) / (LATENCY_MAX_MS - LATENCY_REFERENCE_MS);
        score.clamp(0.0, 1.0)
    }
    ...
    pub fn total_requests(&self) -> u64 {
        self.request_metrics.values().map(|m| m.total_requests()).sum()
    }
...
mod tests {
    ...
    fn test_legacy_score_conversion() {
        ...
        let mut score_with_low_reliability = PeerScore::from_legacy_score(-50, config.clone());

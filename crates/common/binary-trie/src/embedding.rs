...
/// code hash, storage slots `0`-`63` and code chunks `0`-`127`.
///
...
/// [`CODE_OFFSET`]`- 1`, which is `64..=127` — a range that begins on a
/// power of two and is one long, so every sub-index byte in it and no
/// other begins with the bits `01`. The stem plus those two bits is
/// therefore a single prefix over exactly the account's header storage,
/// which is what lets "does this account have storage" be one traversal
/// rather than 64 lookups.
pub fn get_tree_prefix_for_header_storage(address: &Address32) -> KeyPrefix {
    ...
    debug_assert_eq!(CODE_OFFSET, 0b1000_0000);
...
mod tests {
    ...
    fn the_header_prefix_covers_every_header_leaf_and_nothing_else() {
        ...
        // Neither overflow zone belongs to it: overflow code is shared
        // between accounts and overflow storage is its own subtree.
        assert!(!covers(
            &prefix,
            &get_tree_key_for_code_chunk(&a32, &[0x11; 32], 128)
        ));
        assert!(!covers(
            &prefix,
    ...
    fn the_header_storage_prefix_covers_slots_0_to_63_and_no_other_leaf() {
        ...
        // Every other header leaf falls outside: basic data, code hash,
        // and all 128 header code chunks.
        for sub_index in 0..=u8::MAX {
            let in_storage = (HEADER_STORAGE_OFFSET..CODE_OFFSET).contains(&u64::from(sub_index));
            assert_eq!(
                covers(&prefix, &get_tree_key_for_header(&a32, sub_index)),
                in_storage,
                "sub-index {sub_index}"

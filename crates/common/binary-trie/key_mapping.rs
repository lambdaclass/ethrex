...
pub fn get_tree_key_for_code_chunk(address: &Address, chunk_id: u64) -> [u8; 32] {
    ...
    // Safe: result is pos % 256, guaranteed 0–255, fits in u64 and u8.
    let sub_index =
        u64::try_from(pos % *STEM_SUBTREE_WIDTH_U256).expect("pos % 256 is always 0-255") as u8;
...
pub fn get_tree_key_for_storage_slot(address: &Address, storage_key: U256) -> [u8; 32] {
    ...
    if storage_key < header_capacity {
        ...
        // Safe: result is storage_key % 256, guaranteed 0–255, fits in u64 and u8.
        let sub_index = u64::try_from(storage_key % *STEM_SUBTREE_WIDTH_U256)
            .expect("storage_key % 256 is always 0-255") as u8;

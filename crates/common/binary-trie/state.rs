...
use crate::{
    ...
    merkle::{merkelize, stem_node_hash},
    node::Node,
    node::NodeId,
...
// See the reserved-key registry in `node_store.rs` for the full list.
// b'B' (0x42) and b'H' (0x48) are distinct from b'R', b'N', b'h' already claimed there.
...
impl BinaryTrieState {
    ...
    /// Walk the tree and collect dirty nodes (those with `cached_hash == None`)
    /// bucketed by depth (distance from root).
    ///
    /// Returns a `Vec` indexed by depth; `result[d]` contains node IDs at depth
    /// `d` with `cached_hash == None`. Depth 0 is the root; StemNodes are their
    /// own "leaves" of the outer tree (internal structure not traversed further).
    ///
    /// Serial walk; called once before `merkelize_parallel`.
    pub fn collect_dirty_levels(&self) -> Vec<Vec<NodeId>> {
        let mut levels: Vec<Vec<NodeId>> = Vec::new();
        let Some(root_id) = self.trie.root else {
            return levels;
        };
        // BFS over InternalNodes; StemNodes are leaves of the outer tree.
        let mut current_level = vec![root_id];
        while !current_level.is_empty() {
            let mut dirty_at_depth = Vec::new();
            let mut next_level = Vec::new();

            for id in &current_level {
                match self.trie.store.get(*id) {
                    Ok(Node::Internal(internal)) => {
                        // Prune at clean nodes. Invariant: the mutation path
                        // invalidates `cached_hash` on every ancestor of a
                        // modified node (`update_dirty_child` /
                        // `invalidate_dirty_hash`). So a clean InternalNode
                        // (`cached_hash.is_some()`) implies every descendant
                        // is also clean, and we can safely skip the subtree.
                        // This turns collect_dirty_levels from O(total nodes)
                        // into O(dirty frontier).
                        if internal.cached_hash.is_none() {
                            dirty_at_depth.push(*id);
                    ...
                    Ok(Node::Stem(stem)) => {
                        if stem.cached_hash.is_none() {
                            dirty_at_depth.push(*id);
                        }
                        // StemNodes have no outer children to recurse into.
                    }
                    Err(_) => {
                        // Node not found — skip; merkelize will handle the error.
                    }
                }
            }

            // Only push non-empty levels; empty levels would be skipped by
            // merkelize_parallel anyway and waste allocations on deep trees.
            if !dirty_at_depth.is_empty() {
                levels.push(dirty_at_depth);
            }
    ...
    /// Compute the state root using level-parallel merkelization.
    ///
    /// # Design
    ///
    /// `collect_dirty_levels` does a serial BFS to bucket dirty nodes by depth,
    /// pruning at clean subtrees (invariant: every mutation clears `cached_hash`
    /// on all ancestors, so a clean parent implies a clean subtree). We then
    /// process levels bottom-up (deepest first, root last). Within each level
    /// we `par_iter` over dirty node IDs: each thread reads the node's
    /// children's `cached_hash` values (already populated because children's
    /// level was processed earlier) and computes the parent's hash. Results
    /// are collected into a `Vec<(NodeId, [u8; 32])>` via `par_iter().filter_map()
    /// .collect()` — rayon handles the per-thread collection and concatenation
    /// without any explicit mutex. After the level finishes we apply the
    /// collected hashes back into each node's `cached_hash` in a serial pass
    /// using `take`/`put_clean`.
    ///
    /// Correctness: within a single level, no two nodes are ancestors of each
    /// other, so their hash computations are independent and can run in parallel
    /// without data races. The serial apply after each level ensures
    /// `cached_hash` is visible to all threads before the next level starts.
    ///
    /// Falls back to the serial `state_root()` path if the trie is empty.
    pub fn merkelize_parallel(&mut self, pool: &rayon::ThreadPool) -> [u8; 32] {
        if self.trie.root.is_none() {
            return crate::merkle::ZERO_HASH;
        }

        let dirty_levels = self.collect_dirty_levels();

        if dirty_levels.is_empty() {
            // All nodes already have cached hashes; just read the root.
            if let Some(root_id) = self.trie.root
                && let Ok(node) = self.trie.store.get(root_id)
            {
                let h = match node {
                    Node::Internal(internal) => internal.cached_hash,
                    Node::Stem(stem) => stem.cached_hash,
                };
                if let Some(hash) = h {
                    return hash;
                }
            }
            // No cached hash at root despite empty dirty list — fall back to serial.
            return merkelize(&mut self.trie);
        }

        // Process levels bottom-up (deepest level first, root level last).
        // For each level, compute hashes in parallel, then apply them serially.
        let max_depth = dirty_levels.len();
        for depth in (0..max_depth).rev() {
            let node_ids = &dirty_levels[depth];
            if node_ids.is_empty() {
                continue;
            }

            // Parallel phase: compute hashes for all dirty nodes at this depth.
            // Each node reads its children's cached_hash values (already set by
            // prior iterations over deeper levels) and computes its own hash.
            // Results are collected into a lock-free Vec via par_iter().filter_map().collect().
            let computed: Vec<(NodeId, [u8; 32])> = pool.install(|| {
                node_ids
                    .par_iter()
                    .filter_map(|&id| self.compute_node_hash_for_parallel(id).map(|h| (id, h)))
                    .collect()
            });

            // Serial phase: drain computed hashes into each node's cached_hash.
            for (id, hash) in computed {
                match self.trie.store.take(id) {
                    Ok(mut node) => {
                        match &mut node {
                            Node::Internal(internal) => internal.cached_hash = Some(hash),
                            Node::Stem(stem) => stem.cached_hash = Some(hash),
                        }
                        self.trie.store.put_clean(id, node);
                    }
                    Err(_) => {
                        // Node disappeared — fall back to serial for safety.
                        return merkelize(&mut self.trie);
                    }
                }
            }
        ...
        // Root still uncached after parallel pass — fall back to serial.
        merkelize(&mut self.trie)
    }

    /// Compute the hash of a single node by reading its children's cached hashes.
    ///
    /// Used by `merkelize_parallel`'s parallel phase. Children at a deeper level
    /// are already hashed by this point (processed in a prior iteration).
    ///
    /// Returns `None` if any required child hash is missing or the node is absent.
    fn compute_node_hash_for_parallel(&self, id: NodeId) -> Option<[u8; 32]> {
        let node = self.trie.store.get(id).ok()?;
        match node {
            Node::Internal(internal) => {
                // Read children's cached hashes (already computed by deeper levels).
                let left_hash = if let Some(left_id) = internal.left {
                    let child = self.trie.store.get(left_id).ok()?;
                    match child {
                        Node::Internal(c) => c.cached_hash?,
                        Node::Stem(c) => c.cached_hash?,
                    }
                } else {
                    crate::merkle::ZERO_HASH
                };
                let right_hash = if let Some(right_id) = internal.right {
                    let child = self.trie.store.get(right_id).ok()?;
                    match child {
                        Node::Internal(c) => c.cached_hash?,
                        Node::Stem(c) => c.cached_hash?,
                    }
                ...
                buf[32..].copy_from_slice(&right_hash);
                Some(crate::merkle::merkle_hash_64_pub(&buf))
            }
            Node::Stem(stem_node) => {
                // Compute the full stem hash via sparse subtree hashing.
                Some(stem_node_hash(&stem_node))
            }
        }
    }
    ...
    /// # Entry semantics
    ///
    /// Entries are in insertion order. A key MAY appear multiple times if it
    /// was written more than once within the same block (e.g. `SSTORE 1` then
    /// `SSTORE 2` to the same slot). `BinaryMerkleizer::feed_updates` in
    /// standard mode deduplicates at the `AccountUpdate` / slot level before
    /// calling `apply_account_update`, so in practice duplicates only arise
    /// in `new_bal` mode when the caller does not pre-deduplicate. Downstream
    /// FKV consumers (Phase 5's `binary_commit_nodes_to_disk`) MUST apply
    /// **last-write-wins** semantics when inserting into `BINARY_FLATKEYVALUE`.
    ...
    pub(crate) fn record_insert(&mut self, key: [u8; 32], value: [u8; 32]) {
        self.current_block_diffs.push((key, Some(value)));
    }

    /// Record a trie remove into the current block's diffs.
    pub(crate) fn record_remove(&mut self, key: [u8; 32]) {
    ...
    /// Returns `true` if the given tree key has any pending FKV diff in the
    /// current (uncommitted) block — including explicit zero markers recorded
    /// for SSTORE 0. Used by `BinaryBackend::slot_is_in_overlay` so the
    /// overlay-vs-base read path can distinguish "explicitly zeroed in this
    /// session" from "absent" before the diffs are persisted to FKV.
    pub fn has_pending_fkv_entry(&self, tree_key: &[u8; 32]) -> bool {
        self.current_block_diffs.iter().any(|(k, _)| k == tree_key)
    }
    ...
    /// Drain dirty nodes + freed IDs from the underlying `NodeStore` into
    /// serialized `(key, value)` pairs, plus `META_ROOT`, `META_NEXT_ID`, and
    /// `META_ROOT_HASH`.
    ///
    /// `root_hash` is the 32-byte Merkle root computed by the caller (via
    /// `state_root()` / `merkelize_parallel()`). It is stored as `META_ROOT_HASH`
    /// so that `StoreBinaryTrieProvider::load_current_root_hash` can later verify
    /// that the on-disk state matches the `H256` root expected by a reader.
    ///
    /// An empty `value` indicates deletion of that key. After this call the
    /// dirty and freed sets are cleared (nodes are moved to the warm tier).
    ///
    /// Used by `BinaryBackend::commit` to populate `NodeUpdates::Binary.node_diffs`.
    pub fn take_trie_dirty(&mut self, root_hash: [u8; 32]) -> Vec<(Vec<u8>, Vec<u8>)> {
        self.trie.store.take_dirty(self.trie.root, root_hash)
    }
    ...
    pub fn apply_account_update(&mut self, update: &AccountUpdate) -> Result<(), BinaryTrieError> {
        ...
        if !update.added_storage.is_empty() {
            ...
            for (key, value) in &update.added_storage {
                ...
                if value.is_zero() {
                    // Remove the leaf from the trie (state-root correctness:
                    // EIP-7864 treats zero values as absent for hashing).
                    self.trie.remove(tree_key)?;
                    // Record an explicit zero marker in the FKV diff (NOT a
                    // delete). This preserves "explicitly zeroed" vs "absent"
                    // distinction needed by the overlay (transition) read path.
                    // For standalone binary mode this is a no-op semantically:
                    // BinaryBackend::storage reads from the trie which correctly
                    // returns zero either way.
                    self.record_insert(tree_key, [0u8; 32]);

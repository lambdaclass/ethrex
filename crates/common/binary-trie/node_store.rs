...
// ---------------------------------------------------------------------------
// Reserved 0xFF-prefixed meta keys in BINARY_TRIE_NODES
//
// All metadata is stored alongside node data in the BINARY_TRIE_NODES table.
// The 0xFF prefix guarantees no collision with 8-byte u64 LE NodeId keys
// (which have no prefix) or with 32-byte tombstone keys (0xFE prefix).
//
// Registry — every second byte must be unique:
//   [0xFF, b'R'] (0x52) — META_ROOT      : 8-byte LE NodeId of the trie root
//   [0xFF, b'N'] (0x4E) — META_NEXT_ID   : 8-byte LE next allocatable NodeId
//   [0xFF, b'h'] (0x68) — META_ROOT_HASH : 32-byte H256 state root of last commit
//   [0xFF, b'B'] (0x42) — META_BLOCK_KEY : 8-byte LE block number of last flush
//   [0xFF, b'H'] (0x48) — META_BASE_HASH : 32-byte H256 block hash of last flush
//
// NOTE: b'h' (0x68) and b'H' (0x48) differ in ASCII case and are distinct keys.
// When adding a new meta key, claim a new second byte here and document its type.
// ---------------------------------------------------------------------------

/// `BINARY_TRIE_NODES` meta key: 8-byte LE `NodeId` of the current trie root.
///
/// See the meta-key registry comment above for the full list of reserved keys.
pub const META_ROOT: &[u8] = &[0xFF, b'R'];

/// `BINARY_TRIE_NODES` meta key: 8-byte LE next allocatable `NodeId`.
///
/// See the meta-key registry comment above for the full list of reserved keys.
pub const META_NEXT_ID: &[u8] = &[0xFF, b'N'];

/// `BINARY_TRIE_NODES` meta key: 32-byte `H256` state root of the last committed block.
///
/// Written by `take_dirty` alongside `META_ROOT` so that
/// `StoreBinaryTrieProvider::load_current_root_hash` can verify that the
/// on-disk state matches the `H256` root expected by a reader.
///
/// See the meta-key registry comment above for the full list of reserved keys.
pub const META_ROOT_HASH: &[u8] = &[0xFF, b'h'];
...
impl NodeStore {
    ...
    /// Drain all dirty nodes and freed IDs into serialized `(key, value)` pairs,
    /// plus three meta keys: `META_ROOT`, `META_NEXT_ID`, and `META_ROOT_HASH`.
    ///
    /// An empty `value` signals deletion of that key.
    ///
    /// - `root`: the current root `NodeId` (opaque u64).
    /// - `root_hash`: the 32-byte Merkle root hash computed from the trie nodes.
    ///   Stored as `META_ROOT_HASH` so that `StoreBinaryTrieProvider::load_current_root_hash`
    ///   can check whether the on-disk state matches the `H256` root expected by a reader.
    ///
    /// After this call the dirty and freed sets are cleared; the nodes are
    /// moved into the warm tier (matching `collect_flush_ops` behaviour).
    /// Used by `BinaryBackend::commit` to build `NodeUpdates::Binary.node_diffs`.
    pub fn take_dirty(
        &mut self,
        root: Option<NodeId>,
        root_hash: [u8; 32],
    ) -> Vec<(Vec<u8>, Vec<u8>)> {
        let mut out = Vec::with_capacity(self.dirty_nodes.len() + self.freed.len() + 3);

        // Serialize dirty nodes.
        for (id, node) in &self.dirty_nodes {
            out.push((node_key(*id).to_vec(), serialize_node(node)));
        }
        // Serialize freed nodes as deletions (empty value).
        for &id in &self.freed {
            out.push((node_key(id).to_vec(), Vec::new()));
        }

        // Root meta keys.
        out.push((META_ROOT.to_vec(), root.unwrap_or(0).to_le_bytes().to_vec()));
        out.push((META_NEXT_ID.to_vec(), self.next_id.to_le_bytes().to_vec()));
        out.push((META_ROOT_HASH.to_vec(), root_hash.to_vec()));

        // Rotate generations so the caller doesn't see stale dirty state.
        self.rotate_generations();
        self.freed.clear();

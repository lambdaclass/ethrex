...
pub fn blake2b_f(mut r: usize, h: &mut [u64; 8], m: &[u64; 16], t: &[u64; 2], f: bool) {
    ...
    if r > 0 {
        ...
        'process: {
            ...
            inner(
                &mut a, &mut b, &mut c, &mut d, r0a, r0b, r0c, r0d, ror24, ror16,
            );
            ...
            r8a = _mm256_blend_epi32::<0xF0>(
                _mm256_unpacklo_epi64(m3, m7),
                _mm256_alignr_epi8::<8>(m0, m5),
            );
            r8b = _mm256_blend_epi32::<0xF0>(
                _mm256_unpackhi_epi64(m7, m4),
                _mm256_alignr_epi8::<8>(m4, m1),
            );
            r8c = _mm256_blend_epi32::<0xF0>(
                _mm256_unpacklo_epi64(m5, m6),
                _mm256_unpackhi_epi64(m6, m0),
            );
            r8d = _mm256_blend_epi32::<0xF0>(
                _mm256_alignr_epi8::<8>(m1, m2),
                _mm256_alignr_epi8::<8>(m2, m3),
            );
            inner(
                &mut a, &mut b, &mut c, &mut d, r8a, r8b, r8c, r8d, ror24, ror16,
            );
            r = match r.checked_sub(1) {
                Some(x) => x,
                None => break 'process,
            ...
            loop {
                ...
                inner(
                    &mut a, &mut b, &mut c, &mut d, r1a, r2b, r2c, r2d, ror24, ror16,
                );
                r = match r.checked_sub(1) {
                    Some(x) => x,
                    None => break 'process,

...
///
/// All method bodies live as free functions in this module. The trait
/// defaults in [`Crypto`] call them, so `NativeCrypto` needs no overrides.
#[derive(Debug)]
pub struct NativeCrypto;

impl Crypto for NativeCrypto {}

// ── Free-function implementations (called by trait defaults) ─────────────

pub fn secp256k1_ecrecover_impl(
    sig: &[u8; 64],
    recid: u8,
    msg: &[u8; 32],
...
pub fn recover_signer_impl(sig: &[u8; 65], msg: &[u8; 32]) -> Result<Address, CryptoError> {
...
pub fn sha256_impl(input: &[u8]) -> [u8; 32] {
    sha2::Sha256::digest(input).into()
}

pub fn ripemd160_impl(input: &[u8]) -> [u8; 32] {
...
pub fn bn254_g1_add_impl(p1: &[u8], p2: &[u8]) -> Result<[u8; 64], CryptoError> {
    use ark_bn254::Fq;
...
pub fn bn254_g1_mul_impl(point: &[u8], scalar: &[u8]) -> Result<[u8; 64], CryptoError> {
...
pub fn bn254_pairing_check_impl(pairs: &[(&[u8], &[u8])]) -> Result<bool, CryptoError> {
...
pub fn modexp_impl(base: &[u8], exp: &[u8], modulus: &[u8]) -> Result<Vec<u8>, CryptoError> {
...
pub fn secp256r1_verify_impl(msg: &[u8; 32], sig: &[u8; 64], pk: &[u8; 64]) -> bool {
    use p256::{
        EncodedPoint,
...
pub fn bls12_381_pairing_check_impl(
...
pub fn bls12_381_fp_to_g1_impl(fp: &[u8; 48]) -> Result<[u8; 96], CryptoError> {
...
pub fn bls12_381_fp2_to_g2_impl(fp2: ([u8; 48], [u8; 48])) -> Result<[u8; 192], CryptoError> {

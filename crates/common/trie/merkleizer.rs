...
// ---------------------------------------------------------------------------
// TrieProvider trait (MPT-specific, not in shared traits)
// ---------------------------------------------------------------------------

/// Produces `Trie` handles for the state trie (per shard worker) and the
/// per-account storage tries (on-demand during merkleization and on reads
/// inside `MptBackend`).
///
/// # Why this is a trait
///
/// `MptMerkleizer` and `MptBackend` live in `ethrex-trie` and need to call
/// back into the storage layer to open tries backed by on-disk nodes. But
/// `ethrex-trie` sits below `ethrex-storage` in the dep graph (so that future
/// non-MPT backends can depend on trie-side traits without pulling `Store`).
/// The storage layer cannot be named from `ethrex-trie`.
///
/// This trait is the dep-inversion seam: it declares the minimum interface
/// the MPT merkleizer/backend needs, and `ethrex-storage` provides the real
/// implementation (`StoreTrieProvider`) that reads from the DB. There is
/// also a no-op `EmptyTrieProvider` in `mpt_backend.rs` for genesis /
/// witness paths where tries are pre-loaded and the opener is never called.
///
/// # Called where
///
/// - `open_state_trie`: once per shard worker on startup (16x per block)
///   and once inside `finalize` to collapse the final root.
/// - `open_storage_trie`: lazily per storage-modifying account. Cached
///   per-prefix inside each worker so subsequent slots hit the cache.
pub trait TrieProvider: Send + Sync {
    /// Open the state trie rooted at `root`. Implementations must return a
    /// fresh `Trie` handle each call — shard workers need independent
    /// mutable state (`pending_removal`, `dirty`).
    fn open_state_trie(&self, root: H256) -> Result<Trie, TrieError>;

    /// Open the storage trie for `account_hash` rooted at `storage_root`.
    /// Called lazily by workers when they hit a storage-modifying account.
...
pub struct MptMerkleizer {
    ...
    // Trie provider (opens state + storage tries on demand), retained for finalize.
...
impl MptMerkleizer {
    ...
    fn finalize_standard(mut self) -> Result<MerkleOutput, StateError> {
        ...
        let collapsed = collapse_root_node(&|| provider.open_state_trie(parent_state_root), root)
                ...
                if *received == expected_shards.get(&prefix).copied().unwrap_or(0) {
                    ...
                    let new_storage_root = if let Some(mut root) = state.storage_root {
                        ...
                        let collapsed = collapse_root_node(
                            &|| provider.open_storage_trie(prefix, existing_storage_root),
                            *root,
                        )
                        .map_err(|e| StateError::Trie(e.to_string()))?;
...
fn get_or_open_storage_trie<'a>(
    ...
    match storage_tries.entry(prefix) {
        ...
        Entry::Vacant(e) => Ok(e.insert(
            provider
                .open_storage_trie(prefix, storage_root)
                .map_err(|e| StateError::Trie(e.to_string()))?,
        )),
    }
}

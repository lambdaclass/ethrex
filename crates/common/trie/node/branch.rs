...
impl BranchNode {
    ...
    /// Batch-inserts sorted updates into the subtrie rooted at this branch.
    /// Groups updates by first nibble, loads each child once, and recurses.
    pub fn insert_batch(
        &mut self,
        db: &dyn TrieDB,
        ...
        // Partition updates by first nibble. Since input is sorted, keys with
        // the same first nibble are contiguous — we scan for boundaries.
        let mut i = 0;
        while i < updates.len() {
            let mut path_copy = updates[i].0.clone();
            let choice = path_copy.next_choice();

            if let Some(choice_idx) = choice {
                // Find the end of this nibble group
                let group_start = i;
                i += 1;
                while i < updates.len() {
                    let mut p = updates[i].0.clone();
                    if p.next_choice() != Some(choice_idx) {
                        break;
                    }
                    i += 1;
                }

                // Build the sub-updates with paths offset by 1 nibble
                let sub_updates: Vec<(Nibbles, ValueRLP)> = updates[group_start..i]
                    .iter()
                    .map(|(path, value)| {
                ...
                if !self.choices[choice_idx].is_valid() {
                    // No existing child — create a leaf from the first entry
                    let (first_path, first_value) = &sub_updates[0];
                    let new_leaf = LeafNode::new(first_path.clone(), first_value.clone());
                    self.choices[choice_idx] = Node::from(new_leaf).into();
                    // Batch-insert remaining into the new child
                    if sub_updates.len() > 1 {
                        let child = self.choices[choice_idx]
                            .get_node_mut(db, sub_updates[1].0.current())?
                            .ok_or_else(|| {
                                TrieError::InconsistentTree(Box::new(
                                    InconsistentTreeError::RootNotFoundNoHash,
                                ))
                            })?;
                        child.insert_batch(db, &sub_updates[1..])?;
                    }
                } else {
                ...
                self.choices[choice_idx].clear_hash();
            } else {
                // Path exhausted at this branch — update branch value

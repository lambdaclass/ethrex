...
impl TrieState {
    ...
    pub(crate) fn get_node_key(&self, hash: NodeHash) -> Result<CacheKey, TrieError> {
        todo!()
    }
    ...
    pub fn get_node(&mut self, hash: NodeHash) -> Result<Option<Node>, TrieError> {
        ...
        if let NodeHash::Inline(_) = hash {
            Ok(Some(Node::decode_raw(hash.as_ref(), self)?))
        } else if let Some(node) = self.cache.get(&hash) {
            Ok(Some(node.clone()))
        } else {
            self.db
                .get(hash.into())?
                .map(|rlp| {
                    let key = NodeDecoder::decode(&rlp)
                        .map(|decoder| decoder.finish(todo!(), todo!()))
    ...
    // ) -> Result<(), TrieError> {
    //     let Some(node) = self.cache.remove(key) else {
    ...
    //     }
    //     // Commit self
    //     acc.push((key.into(), NodeEncoder(&node, self).encode_to_vec()));
    ...
    pub fn write_node(&mut self, node: Node, hash: NodeHash) -> Result<(), TrieError> {
        ...
        if matches!(hash, NodeHash::Hashed(_)) {
            self.db
                .put(hash.into(), NodeEncoder(&node, self).encode_to_vec())?;
        }
        Ok(())
    }
        ...
        let key_values = nodes
            ...
            .filter_map(|node| {
                ...
                matches!(hash, NodeHash::Hashed(_))
                    .then(|| (hash.into(), NodeEncoder(&node, self).encode_to_vec()))
            })
            .collect();

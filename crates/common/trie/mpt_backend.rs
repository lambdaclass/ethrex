...
use ethrex_state_backend::{
    AccountMut, CodeMut, MerkleOutput, NodeUpdates, StateCommitter, StateError, StateReader,
};

use crate::guest_program_state::collect_accounts_from_trie;
use crate::mpt_merkleizer::StorageTrieOpener;
use crate::{
    EMPTY_TRIE_HASH, Nibbles, Node, NodeHash, NodeRef, Trie, TrieError, TrieLogger, TrieWitness,
};

/// Closure type for reading code by hash from the DB.
pub type CodeReader = Arc<dyn Fn(H256) -> Result<Option<Vec<u8>>, TrieError> + Send + Sync>;
...
/// MPT-specific: binary trie uses a different key derivation.
pub fn mpt_hash_address(address: &Address) -> Vec<u8> {
    keccak_hash(address.to_fixed_bytes()).to_vec()
}

/// Keccak-hash a storage key to derive the MPT storage trie key.
/// MPT-specific: binary trie uses a different key derivation.
pub fn mpt_hash_key(key: &H256) -> Vec<u8> {
    keccak_hash(key.to_fixed_bytes()).to_vec()
}

// ---- No-op implementations for in-memory / witness paths ----

/// Returns empty tries for accounts not already in the cache.
/// Used by genesis (no existing state) and witness (all tries pre-loaded).
struct EmptyStorageTrieOpener;

impl StorageTrieOpener for EmptyStorageTrieOpener {
    fn open(&self, _account_hash: H256, _storage_root: H256) -> Result<Trie, TrieError> {
        Ok(Trie::default())
    }
}
...
fn no_op_code_reader(_hash: H256) -> Result<Option<Vec<u8>>, TrieError> {
    Ok(None)
}
...
pub struct MptWitnessState {
    ...
    pub storage_tries: MptStorageTries,
}
...
pub struct MptBackend {
    ...
    pub witness_state: Option<MptWitnessState>,
}
...
impl MptBackend {
    ...
    pub fn new(state_trie: Trie, crypto: Arc<dyn Crypto>) -> Self {
        Self {
            ...
            codes: BTreeMap::new(),
            crypto,
            storage_opener: Arc::new(EmptyStorageTrieOpener),
    ...
    fn hash_slot(&self, slot: &H256) -> Vec<u8> {
        self.crypto.keccak256(&slot.to_fixed_bytes()).to_vec()
    }
    ...
    pub fn storage_with_hint(
        ...
        let storage_trie = match self.storage_tries.get(&hashed_addr) {
            ...
            None => {
                ...
                let trie = self.storage_opener.open(hashed_addr, storage_root)?;
    ...
    ///
    /// This is the backend-internal equivalent of the old
    /// `Store::apply_account_updates_from_trie_batch`. It uses `self.state_trie`
    /// and `self.storage_opener` for all trie access, keeping hash function and
    /// encoding internal to MPT.
    pub fn apply_updates_with_output(
        &mut self,
        ...
        for update in account_updates {
            ...
            if !update.added_storage.is_empty() {
                // Open storage trie. If storage was wiped, always start fresh
                // to avoid reusing stale nodes from a pre-loaded trie.
                let mut storage_trie = if update.removed_storage {
                    let _ = self.storage_tries.remove(&hashed_address);
                    Trie::default()
                } else if let Some(trie) = self.storage_tries.remove(&hashed_address) {
                    trie
                } else {
                    self.storage_opener
                        .open(hashed_address, account_state.storage_root)?
                };
                ...
                let (storage_hash, storage_nodes) =
        ...
        let (state_trie_hash, state_updates) = self
            .state_trie
        ...
        for update in account_updates {
            ...
            if !update.added_storage.is_empty() {
                ...
                let (_witness, storage_trie) = match storage_tries.entry(update.address) {
                    ...
                    Entry::Vacant(vac) => {
                        let raw_trie = if update.removed_storage {
                            Trie::default()
                        } else {
                            self.storage_opener
                                .open(hashed_address, account_state.storage_root)
    ...
    pub fn record_witness_storage(
        ...
        let hashed_keys: Vec<Vec<u8>> = keys.iter().map(|k| self.hash_slot(k)).collect();
    ...
    pub fn finalize_witness(
        ...
        for address in touched_accounts.keys() {
            let hashed_address = mpt_hash_address(address);
    ...
    pub fn flush_storage_roots(&mut self) -> Result<(), StateError> {
        for (hashed_addr, trie) in &self.storage_tries {
            ...
            // without a matching account, that invariant is broken.
...
impl StateReader for MptBackend {
    ...
    fn storage(&self, addr: Address, slot: H256) -> Result<H256, StateError> {
        ...
        let storage_trie = self.storage_opener.open(hashed_addr, storage_root)?;
        let hashed_slot = self.hash_slot(&slot);
        Self::read_storage_value(&storage_trie, &hashed_slot)
    }
    ...
    fn code(&self, _addr: Address, code_hash: H256) -> Result<Option<Vec<u8>>, StateError> {
        ...
        (self.code_reader)(code_hash).map_err(StateError::from)
    }
}
        ...
        for (addr, acct_mut) in addrs.iter().zip(muts.iter()) {
            ...
            match &acct_mut.account {
                None => {
                    ...
                    self.storage_root_cache.lock().unwrap().remove(&hashed);
                }
                Some(info) => {

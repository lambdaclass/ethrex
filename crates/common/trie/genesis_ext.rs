...
/// Build a genesis [`BlockHeader`] from the genesis configuration.
pub fn genesis_header(genesis: &Genesis) -> BlockHeader {
    ...
    BlockHeader {
        ...
        state_root: genesis_compute_state_root(genesis),
...
/// Build a genesis [`BlockBody`].
pub fn genesis_body() -> BlockBody {
    BlockBody {
        transactions: vec![],
        ommers: vec![],
...
/// Compute the MPT state root for a genesis configuration.
pub fn genesis_compute_state_root(genesis: &Genesis) -> H256 {

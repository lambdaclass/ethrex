...
fn add_center_to_parent_and_write_queue(
    ...
    nodes_to_write.push((parent_element.path.append_new(index), node));
    Ok(())
}
    ...
    T: Iterator<Item = (H256, Vec<u8>)> + Send,
{
    ...
    if right_side_opt.is_none() {
        ...
        let hash = node.compute_hash();
        flush_nodes_to_write(vec![(Nibbles::default(), node.into())], db, buffer_sender)?;
        ...
        == 1
    {
        ...
        debug_assert!(nodes_to_write.last().unwrap().1.compute_hash() == child.compute_hash());
        ...
        nodes_to_write.push((left_side.path, node));
        nodes_to_write
            .last()
            .expect("we just inserted")
            .1
...
where
    T: Iterator<Item = (H256, Vec<u8>)> + Send,
{
    let (buffer_sender, buffer_receiver) = bounded(BUFFER_COUNT as usize);

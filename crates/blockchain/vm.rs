...
pub struct StoreVmDatabase {
    ...
    // Cache account storage roots to avoid repeated state trie lookups per slot read.
    pub storage_root_cache: Arc<Mutex<FxHashMap<Address, Option<H256>>>>,
    pub state_root: H256,
}
    ...
    fn get_storage_slot(&self, address: Address, key: H256) -> Result<Option<U256>, EvmError> {
        ...
        let storage_root = match cached_root {
            ...
            None => {
                ...
                match account_state {
                    Some(state) => {
                        cache.insert(address, Some(state.storage_root));
                        state.storage_root
                    }
                    None => {
                        cache.insert(address, None);
                        return Ok(None);
                    }
                }
            }
        ...
        self.store
            .get_storage_at_root_with_storage_root(self.state_root, address, storage_root, key)
            .map_err(|e| EvmError::DB(e.to_string()))
    }

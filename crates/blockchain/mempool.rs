...
impl Mempool {
    ...
    pub fn evict_stale(&self, ttl: Duration) -> Result<usize, MempoolError> {
        ...
        // Single write lock for the whole scan, unlike `evict_dormant`'s
        // snapshot-then-evict split. That split exists because `evict_dormant`
        // performs async storage lookups between the two phases, which must not
        // happen under the lock — and it pays for it with a TOCTOU window it has
        // to close by re-checking timestamps. This function does no I/O at all:
        // it is a bounded in-memory pass over at most `max_mempool_size`
        // entries, so holding the lock is both cheaper and free of that window.
        let mut inner = self.write()?;
...
mod tests {
    ...
    use ethrex_common::types::{
        EIP1559Transaction, EIP4844Transaction, Genesis, LegacyTransaction, Transaction, TxKind,
    };

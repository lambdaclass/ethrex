...
impl Blockchain {
    ...
    /// `Privileged` 0x7e). Those carry an unsigned, caller-chosen `from` (no signature
    /// recovery) and are unknown to other L1 clients, so accepting one would diverge
    /// consensus and let a block forge a sender. On L2 these types are valid, so this
...
mod l1_transaction_type_tests {
    ...
    //! `PrivilegedL2Transaction` / 0x7d `FeeTokenTransaction`). Such a transaction
    //! carries an unsigned, caller-chosen `from` (no signature recovery) and is unknown
    //! to other L1 clients, so accepting one both diverges consensus and lets a block
    //! forge a sender.

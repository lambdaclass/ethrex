    ...
    */
    ...
    pub async fn validate_transaction(
        ...
        if let Transaction::FrameTransaction(frame_tx) = tx {
            ...
            // EIP-8272 §Mempool freshness policy (SHOULD; local peer policy — it
            // may over-reject but must never under-reject): admit a
            // reference-carrying frame tx only if every declared recent-root
            // reference would be valid in the earliest block that could include
            // it. `current_slot` at admission is therefore `head.slot_number + 1`;
            // a reference that is "too new" or expired against that slot can never
            // make the tx valid right now. The three consensus conditions mirrored
            // here:
            ...
            if !frame_tx.recent_root_references.is_empty()
                && (header.slot_number.is_some()
                    || config.is_derived_slot_activated(header.timestamp))
            {
                ...
                for reference in &frame_tx.recent_root_references {
                    ...
                    let stored = self
                        .storage
                        .get_storage_at(header_no, recent_root_address, reference.storage_key())?
                        .unwrap_or_default();

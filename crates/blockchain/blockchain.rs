...
impl Blockchain {
    ...
    pub async fn add_block(&self, block: &Block) -> Result<(), ChainError> {
        ...
        let apply_updates_list = self
            .storage
            .apply_account_updates_batch(&mut self.state_trie.write().await.1, &updates)
            .await?;
    ...
    fn print_add_block_logs(
        ...
        if interval != 0f64 {
            ...
            let base_log = format!(
                "[METRIC] BLOCK EXECUTION THROUGHPUT: {:.2} Ggas/s. Gas used: {} Ggas. Time spent: {:.0} ms. #Txs: {}.",
                as_gigas,
                throughput,
                interval,
            ...
            let extra_log = if as_gigas > 0.0 {
                let execution_fraction =
                    (executed.duration_since(since).as_millis() as f64 * 100_f64 / interval)
                        .round();
                let merkle_fraction =
                    (merkelized.duration_since(executed).as_millis() as f64 * 100_f64 / interval)
                        .round();
                let storage_fraction =
                    (stored.duration_since(merkelized).as_millis() as f64 * 100_f64 / interval)
                        .round();

                format!(
                    " exec: {}% merkle: {}% store: {}%",
                    execution_fraction, merkle_fraction, storage_fraction
                )
            } else {
                "".to_string()
            };

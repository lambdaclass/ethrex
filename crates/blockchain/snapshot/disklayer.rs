...
use ethrex_common::{
    types::{AccountInfo, BlockHash},
    H256, U256,
};
use ethrex_storage::StoreEngine;
...
impl DiskLayer {
    ...
    pub fn get_account(&self, hash: H256) -> Result<Option<AccountInfo>, SnapshotError> {
        if self.stale() {
        ...
        let account = self.db.get_account_snapshot(hash)?.map(|acc| AccountInfo {
            balance: acc.balance,

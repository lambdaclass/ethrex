...
fn bench_transaction_operations(c: &mut Criterion) {
    ...
    for num_transactions in [10, 50, 100].iter() {
        ...
        // Benchmark InMemory store
        {
            ...
            group.bench_with_input(
                BenchmarkId::new("InMemory", num_transactions),
                num_transactions,
                |b, &num_transactions| {
                    b.iter(|| {
                        let transactions = (0..num_transactions)
                            .map(|i| create_random_transaction(base_seed + i as u64))
                            .collect::<Vec<_>>();
        ...
        #[cfg(feature = "libmdbx")]
        {
            ...
            group.bench_with_input(
                BenchmarkId::new("Libmdbx", num_transactions),
                num_transactions,
                |b, &num_transactions| {
                    b.iter(|| {
                        let transactions = (0..num_transactions)
        ...
        #[cfg(feature = "redb")]
        {
            ...
            group.bench_with_input(
                BenchmarkId::new("RedB", num_transactions),
                num_transactions,
                |b, &num_transactions| {
                    b.iter(|| {
                        let transactions = (0..num_transactions)
...
fn bench_block_retrieval(c: &mut Criterion) {
    let mut group = c.benchmark_group("block_retrieval");
    group.measurement_time(Duration::from_secs(10));

    // Use the same base seed for consistent results
    let base_seed = 42;

    // Setup function to create stores with blocks to retrieve
    fn setup_bench(engine_type: EngineType, base_seed: u64) -> (Store, Vec<BlockHash>, TempDir) {
        let (store, temp_dir) = create_test_store(engine_type);
        let genesis_header = store.get_block_header(0).unwrap().unwrap();
        let genesis_hash = genesis_header.compute_block_hash();
        ...
        (store, block_hashes, temp_dir)
    }

    // InMemory implementation
    {
        let (store, block_hashes, _temp_dir) = setup_bench(EngineType::InMemory, base_seed);

        group.bench_function("InMemory_retrieve_blocks", |b| {
            b.iter(|| {
                for &hash in &block_hashes[0..50] {
                    black_box(store.get_block_by_hash(hash).unwrap());
                }
            });
        });
    }

    // MDBX implementation if available
    #[cfg(feature = "libmdbx")]
    {
        let (store, block_hashes, _temp_dir) = setup_bench(EngineType::Libmdbx, base_seed);

        group.bench_function("Libmdbx_retrieve_blocks", |b| {
            b.iter(|| {
                for &hash in &block_hashes[0..50] {
                    black_box(store.get_block_by_hash(hash).unwrap());
                }
    ...
    // RedB implementation if available
    #[cfg(feature = "redb")]
    {
        let (store, block_hashes, _temp_dir) = setup_bench(EngineType::RedB, base_seed);

        group.bench_function("RedB_retrieve_blocks", |b| {
            b.iter(|| {
                for &hash in &block_hashes[0..50] {
                    black_box(store.get_block_by_hash(hash).unwrap());
                }

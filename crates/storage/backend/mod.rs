...
//!
//! This module contains the trait and implementations for the storage backend.
//! We use this trait to abstract over different database implementations, database
//! should only know about bytes and namespaces.
//!
...
pub trait StorageBackend: Send + Sync + Debug + RefUnwindSafe {
    ...
    /// Get a value by key from the specified namespace. This is an async version of [`StorageBackend::get_sync`].
    async fn get_async(&self, namespace: &str, key: Vec<u8>)
    -> Result<Option<Vec<u8>>, StoreError>;

    /// Get a batch of values by key from the specified namespace.
    ...
    /// Put a key-value pair in the specified namespace. This is an async version of [`StorageBackend::put_sync`].
    ...
    /// Available operations are [`BatchOp::Put`] and [`BatchOp::Delete`].
    /// TODO: Rename this?
    ...
    /// TODO: Should we return an iterator instead?
    async fn range(
        &self,
        namespace: &str,
        start_key: Vec<u8>,

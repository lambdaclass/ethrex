...
pub trait DbCursorRO<T: Table> {
    ...
    fn walk(&mut self, start_key: Option<T::Key>) -> Result<Walker<'_, T, Self>, StoreError>
    where
        Self: Sized;
    ...
    ) -> Result<RangeWalker<'_, T, Self>, StoreError>
    where
        Self: Sized;
    ...
    ) -> Result<ReverseWalker<'_, T, Self>, StoreError>
    where
        Self: Sized;
}
...
pub trait DbDupCursorRO<T: DupSort> {
    ...
    ) -> Result<DupWalker<'_, T, Self>, StoreError>
    where
        Self: Sized;
}
...
pub trait DbCursorRW<T: Table> {
    ...
    fn upsert(&mut self, key: T::Key, value: &T::Value) -> Result<(), StoreError>;
    ...
    fn insert(&mut self, key: T::Key, value: &T::Value) -> Result<(), StoreError>;

    /// Append value to next cursor item.
    ///
    /// This is efficient for pre-sorted data. If the data is not pre-sorted, use
    /// [`DbCursorRW::insert`].
    fn append(&mut self, key: T::Key, value: &T::Value) -> Result<(), StoreError>;
...
pub trait DbDupCursorRW<T: DupSort> {
    ...
    fn delete_current_duplicates(&mut self) -> Result<(), StoreError>;

    /// Append duplicate value.
    ///
    /// This is efficient for pre-sorted data. If the data is not pre-sorted, use `insert`.
    fn append_dup(&mut self, key: T::Key, value: T::Value) -> Result<(), StoreError>;
}
...
impl<T: Table, CURSOR: DbCursorRO<T>> Iterator for Walker<'_, T, CURSOR> {
    type Item = Result<TableRow<T>, StoreError>;

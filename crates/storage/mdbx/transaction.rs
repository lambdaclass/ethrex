...
pub trait DbTx: Debug + Send + Sync {
    ...
    fn commit(self) -> Result<bool, StoreError>;
    /// Aborts transaction
    fn abort(self);
    /// Iterate over read only values in table.
    fn cursor_read<T: Table>(&self) -> Result<Self::Cursor<T>, StoreError>;
    /// Iterate over read only values in dup sorted table.
    fn cursor_dup_read<T: DupSort>(&self) -> Result<Self::DupCursor<T>, StoreError>;
    /// Returns number of entries in the table.
    fn entries<T: Table>(&self) -> Result<usize, StoreError>;
...
pub trait DbTxMut: Send + Sync {
    ...
    /// Clears database.
    fn clear<T: Table>(&self) -> Result<(), StoreError>;
    /// Cursor mut
    fn cursor_write<T: Table>(&self) -> Result<Self::CursorMut<T>, StoreError>;
    /// `DupCursor` mut.
    fn cursor_dup_write<T: DupSort>(&self) -> Result<Self::DupCursorMut<T>, StoreError>;
}

...
impl Store {
    ...
    pub fn begin_storage_read_session(
        &self,
        state_root: H256,
    ) -> Result<StorageReadSession, StoreError> {
        // Gate the snapshot on `state_root` exactly like `gated_snapshot`: a block
        // just added via the deferred persist path may still be pending install in
        // `trie_cache`. Snapshotting eagerly (as before #6905) could capture a cache
        // that lacks this root's layer -> stale reads -> non-VALID execution.
    ...
    pub fn get_account_states_batch_by_root(
        ...
        // Gate on the read root so a still-pending block's layer is installed
        // before we snapshot (see `begin_storage_read_session`).
        let trie_cache = self.gated_snapshot(state_root)?;

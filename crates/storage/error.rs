...
pub enum StoreError {
    ...
    #[error(
        "State not available: block {block} is beyond history depth (head: {head}, max depth: {max_depth}). Only finalized state and recent unfinalized blocks are available."
    )]

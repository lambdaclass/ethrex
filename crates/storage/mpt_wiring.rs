...
//! A binary trie would have a parallel `binary_wiring.rs`.
...
use crate::{
    ...
    apply_prefix,
    error::StoreError,
    layering::TrieWrapper,
    trie::{BackendTrieDB, BackendTrieDBLocked},
};
use ethrex_common::{
    Address, H256, U256,
    types::{AccountInfo, AccountState, BlockHash, BlockNumber, Genesis, GenesisAccount},
};
use ethrex_crypto::NativeCrypto;
use ethrex_rlp::decode::RLPDecode;
use ethrex_trie::{
    EMPTY_TRIE_HASH, ExecutionWitness, Nibbles, Node, StorageTrieOpener, Trie, TrieError,
    TrieWitness, execution_witness_to_rpc, genesis_block,
};
use std::{
...
pub(crate) type StorageTries = HashMap<Address, (TrieWitness, Trie)>;
...
pub(crate) fn hash_address(address: &Address) -> Vec<u8> {
    ethrex_trie::mpt_hash_address(address)
}

pub(crate) fn hash_address_fixed(address: &Address) -> H256 {
    H256::from_slice(&ethrex_trie::mpt_hash_address(address))
}

pub(crate) fn hash_key(key: &H256) -> Vec<u8> {
    ethrex_trie::mpt_hash_key(key)
}
...
/// - key len == 65: account leaf  -> `ACCOUNT_FLATKEYVALUE`
/// - key len == 131: storage leaf -> `STORAGE_FLATKEYVALUE`
/// - key len < 65:  account node  -> `ACCOUNT_TRIE_NODES`
...
// ---------------------------------------------------------------------------
// StorageTrieOpener backed by Store
// ---------------------------------------------------------------------------

/// [`StorageTrieOpener`] implementation backed by a [`Store`].
///
/// Created by [`Store::make_storage_trie_opener`] and passed to
/// [`ethrex_trie::MptMerkleizer::new`].
struct StoreStorageTrieOpener {
    store: Store,
    parent_state_root: H256,
}

impl StorageTrieOpener for StoreStorageTrieOpener {
...
impl Store {
    ...
    /// Create a closure that opens a state trie from the store.
    ///
    /// The returned closure captures store internals and can be passed to
    /// [`ethrex_trie::MptMerkleizer::new`].
    pub fn make_state_trie_opener(
        &self,
        parent_state_root: H256,
    ) -> Arc<dyn Fn() -> Result<Trie, TrieError> + Send + Sync> {
    ...
    /// Create a [`StorageTrieOpener`] that opens storage tries from the store.
    ///
    /// The returned opener can be passed to [`ethrex_trie::MptMerkleizer::new`].
    pub fn make_storage_trie_opener(&self, parent_state_root: H256) -> Arc<dyn StorageTrieOpener> {
        Arc::new(StoreStorageTrieOpener {
            store: self.clone(),
            parent_state_root,
        })
    }
    ...
    pub(crate) fn make_code_reader(
        &self,
    ) -> Arc<dyn Fn(H256) -> Result<Option<Vec<u8>>, TrieError> + Send + Sync> {
        let store = self.clone();
    ...
    pub(crate) fn write_mpt_node_updates(
        ...
        let account_trie_nodes: Vec<(Vec<u8>, Vec<u8>)> = state_updates
            .into_iter()
            .map(|(path, node)| (apply_prefix(None, path), node))
            .collect();
    ...
    pub async fn setup_genesis_state_trie(
        ...
        use ethrex_common::types::code_hash;
    ...
    pub(crate) fn get_account_state_from_trie(
        &self,
        state_trie: &Trie,
        address: Address,
    ) -> Result<Option<AccountState>, StoreError> {
    ...
    pub fn store_witness(
        ...
        let rpc_witness = execution_witness_to_rpc(witness);

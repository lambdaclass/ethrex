...
impl DomainStore {
    ...
    async fn clear_snap_state(&self) -> Result<(), StorageError> {
        ...
        let all_data = self.schema.range(DBTable::SnapState, &[0], None).await?;
    ...
    fn open_locked_state_trie(&self, state_root: H256) -> Result<Trie, StorageError> {
        // For now, same as regular state trie - can be optimized later with actual locking
        todo!()
    }
    ...
    fn open_locked_storage_trie(
        ...
        // For now, same as regular storage trie - can be optimized later with actual locking
        todo!()
    }
    ...
    async fn set_latest_valid_ancestor(
        ...
        self.schema
            .put(DBTable::InvalidAncestors, &key, &value)
            .await
    }
    ...
    fn get_canonical_block_hash_sync(
        ...
        match self
            .schema
            .get_sync(DBTable::CanonicalHashes, &number_key)?
        {
            Some(bytes) => {

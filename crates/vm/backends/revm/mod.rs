...
use ethrex_levm::constants::{BLOB_BASE_FEE_UPDATE_FRACTION, SYS_CALL_GAS_LIMIT, TX_BASE_COST};
use revm::context::transaction::{
    AccessListItem, Authorization as RevmAuthorization, RecoveredAuthorization, SignedAuthorization,
};
...
use revm::primitives::{
    Address as RevmAddress, B256, FixedBytes, TxKind as RevmTxKind, U256 as RevmU256,
    hardfork::SpecId,
};
    ...
    pub fn get_state_transitions(
        ...
        for (address, account) in bundle.state() {
                ...
                && let Some(new_acc_info) = account.account_info()
            {
                ...
                {
                    account_update.code = Some(Code::from_bytecode(code.original_bytes().into()));
                }
            }
...
pub fn tx_env(tx: &Transaction, sender: Address) -> TxEnv {
    TxEnv {
        ...
        gas_priority_fee: tx.max_priority_fee().map(u128::from),
        blob_hashes: tx
...
pub(crate) fn tx_env_from_generic(tx: &GenericTransaction, basefee: u64) -> TxEnv {
    ...
    TxEnv {
        ...
        nonce: tx.nonce.expect("Failed to get transaction nonce"),
        chain_id: tx.chain_id,
        access_list: RevmAccessList(
            tx.access_list
                .iter()
        ...
        gas_priority_fee: tx.max_priority_fee_per_gas.map(u128::from),
        blob_hashes: tx
            .blob_versioned_hashes
            .iter()
            .map(|hash| B256::from(hash.0))
...
//     let current_access_list = tx_env.access_list;
...
fn adjust_disabled_base_fee(block_env: &mut BlockEnv, tx_gas_price: u128, tx_blob_gas_price: u128) {
    if tx_gas_price == 0 {
        block_env.basefee = 0;
    }
    if tx_blob_gas_price == 0 {
...
pub(crate) fn generic_system_contract_revm(
    ...
    Ok(ExecutionResult::from(transaction_result.result))
}

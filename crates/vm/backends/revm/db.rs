...
pub struct EvmState {
    pub inner: revm::database::State<DynVmDatabase>,
}
...
impl Clone for EvmState {
    fn clone(&self) -> Self {
        let inner = revm::database::State::<DynVmDatabase> {
...
impl revm::Database for DynVmDatabase {
    ...
    fn basic(&mut self, address: RevmAddress) -> Result<Option<RevmAccountInfo>, Self::Error> {
        let acc_state = match <dyn VmDatabase>::get_account_state(
            self.as_ref(),
        ...
        let code = self.code_by_hash(acc_state.code_hash.0.into())?;

        Ok(Some(RevmAccountInfo {
            balance: RevmU256::from_limbs(acc_state.balance.0),
            nonce: acc_state.nonce,
            code_hash: RevmB256::from(acc_state.code_hash.0),
            code: Some(code),
        }))
    }

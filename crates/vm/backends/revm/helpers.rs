use ethrex_common::types::{BlockHeader, GenericTransaction, INITIAL_BASE_FEE};
...
use super::{block_env, run_without_commit, tx_env_from_generic};
...
//         let mut combined_access_list = tx_env.access_list.clone();
//         combined_access_list.0.extend(access_list.0.clone());
//         tx_env.access_list = combined_access_list;
...
//     #[allow(unused_mut)]
//     let mut evm_builder = Context::new(state.inner, spec_id)
//         .with_block(block_env)
//         .with_tx(tx_env);

//     evm_builder.modify_cfg(|env| {
//         env.disable_base_fee = true;
//         env.disable_block_gas_limit = true
//     });

//     let mut evm = evm_builder.build_mainnet_with_inspector(access_list_inspector.clone());

...
use crate::backends::revm::db::{EvmState, evm_state};
...
impl Evm {
    ...
    pub fn execute_block(&mut self, block: &Block) -> Result<BlockExecutionResult, EvmError> {
        REVM::execute_block(block, &mut self.db)
    }
    ...
    pub fn execute_tx(
        ...
        let spec_id = spec_id(&self.db.chain_config()?, block_header.timestamp);
        let execution_report = REVM::execute_tx(tx, block_header, &mut self.db, spec_id, sender)?;
    ...
    pub fn apply_system_calls(&mut self, block_header: &BlockHeader) -> Result<(), EvmError> {
        ...
        if block_header.parent_beacon_block_root.is_some() && fork >= Fork::Cancun {
            REVM::beacon_root_contract_call(block_header, &mut self.db)?;
        }

        if fork >= Fork::Prague {
            REVM::process_block_hash_history(block_header, &mut self.db)?;
        }
    ...
    pub fn get_state_transitions(&mut self) -> Result<Vec<AccountUpdate>, EvmError> {
        Ok(REVM::get_state_transitions(&mut self.db))
    }
    ...
    pub fn process_withdrawals(&mut self, withdrawals: &[Withdrawal]) -> Result<(), EvmError> {
        REVM::process_withdrawals(&mut self.db, withdrawals)
    }
    ...
    pub fn extract_requests(
        ...
        // REVM::extract_all_requests(&mut self.db, receipts, header, self.vm_type)
        todo!()
    }
    ...
    pub fn create_access_list(
        ...
        // let result = { REVM::create_access_list(tx.clone(), header, &mut self.db, self.vm_type)? };

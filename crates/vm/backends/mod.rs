...
pub trait Evm {
    ...
    fn new_for_l2(_db: impl VmDatabase + 'static) -> Self;
    ...
    fn create_access_list(
        ...
        let (result, access_list) = self.create_access_list_inner(tx, header, _fork)?;

        match result {
            ExecutionResult::Success { gas_used, .. } => Ok((gas_used, access_list, None)),
            ExecutionResult::Revert { gas_used, .. } => Ok((
                gas_used,
                access_list,
                Some("Transaction Reverted".to_string()),
            )),
            ExecutionResult::Halt { reason, gas_used } => Ok((gas_used, access_list, Some(reason))),
        }
    }

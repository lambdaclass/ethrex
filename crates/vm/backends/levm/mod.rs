...
impl LEVM {
    ...
    /// Validates BAL entries at index 0 (system calls / prepare_block phase)
    /// against the actual post-prepare_block state in `system_seed`.
    ///
    /// For each account with a change at index 0 in the BAL, verify the claimed
    /// post-system-call value matches what prepare_block actually produced.
    fn validate_bal_index_zero(
        bal: &BlockAccessList,
        validation_index: &BalAddressIndex,
        system_seed: &CacheDB,
        store: &Arc<dyn Database>,
    ) -> Result<(), EvmError> {
        let Some(active_accounts) = validation_index.tx_to_accounts.get(&0) else {
            return Ok(()); // No index-0 entries → nothing to validate
        };

        for &acct_inner_idx in active_accounts {
            let acct = &bal.accounts()[acct_inner_idx];
            let addr = acct.address;
            let actual = system_seed.get(&addr);
            ...
            if let Some(expected_balance) = find_exact_change_balance(&acct.balance_changes, 0) {
                match actual {
                    Some(a) if a.info.balance == expected_balance => {
                        // Value matches system_seed — now verify the system call
                        // actually changed it (pre-state must differ).
                        if pre_state().balance == expected_balance {
                            return Err(EvmError::Custom(format!(
                                "BAL validation failed for system_tx: account {addr:?} has \
                                 extraneous balance change at index 0 (value {expected_balance} \
                    ...
                    Some(a) => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} balance \
                             mismatch at index 0: BAL={expected_balance}, actual={}",
                            a.info.balance
                        )));
                    }
                    None => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} has \
                             balance change at index 0 but was not touched by system calls"
                        )));
                    }
                }
            }

            // Nonce change at index 0
            if let Some(expected_nonce) = find_exact_change_nonce(&acct.nonce_changes, 0) {
                match actual {
                    Some(a) if a.info.nonce == expected_nonce => {
                        if pre_state().nonce == expected_nonce {
                            return Err(EvmError::Custom(format!(
                                "BAL validation failed for system_tx: account {addr:?} has \
                                 extraneous nonce change at index 0 (value {expected_nonce} \
                                 unchanged from pre-state)"
                            )));
                        }
                    }
                    Some(a) => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} nonce \
                             mismatch at index 0: BAL={expected_nonce}, actual={}",
                            a.info.nonce
                        )));
                    }
                    None => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} has \
                             nonce change at index 0 but was not touched by system calls"
                        )));
                    }
                }
            }

            // Code change at index 0
            if let Some(expected_code) = find_exact_change_code(&acct.code_changes, 0) {
                ...
                match actual {
                    Some(a) if a.info.code_hash == code_hash => {
                        if pre_state().code_hash == code_hash {
                            return Err(EvmError::Custom(format!(
                                "BAL validation failed for system_tx: account {addr:?} has \
                                 extraneous code change at index 0 (unchanged from pre-state)"
                            )));
                        }
                    }
                    Some(_) => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} code \
                             mismatch at index 0"
                        )));
                    }
                    None => {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} has \
                             code change at index 0 but was not touched by system calls"
                        )));
                    }
                }
            }

            // Storage changes at index 0
            for sc in &acct.storage_changes {
                if let Some(expected_value) = find_exact_change_storage(&sc.slot_changes, 0) {
                    let key = ethrex_common::utils::u256_to_h256(sc.slot);
                    let actual_value = actual.and_then(|a| a.storage.get(&key)).copied();
                    if actual_value != Some(expected_value) {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} storage \
                             slot {} mismatch at index 0: BAL={expected_value}, actual={actual_value:?}",
                            sc.slot
                        )));
                    }
                    ...
                    if pre_slot == expected_value {
                        return Err(EvmError::Custom(format!(
                            "BAL validation failed for system_tx: account {addr:?} has \
                             extraneous storage change for slot {} at index 0 (value \
                             {expected_value} unchanged from pre-state)",
                            sc.slot
                        )));
                    }
                }

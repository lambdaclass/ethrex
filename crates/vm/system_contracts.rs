...
/// roots keyed by (source_id, slot), written by calling it with `salt ‖ root`, and
/// checks `(source_id, slot, root)` tuples against them for the canonical
...
/// Runtime bytecode of the EIP-8272 RECENT_ROOT_ADDRESS predeploy, both operations of
/// the spec's §"Recent root contract" (`824cbc0b0e`), selected by calldata length:
///
/// - **write**, 64 bytes `salt ‖ root` and zero value: stores `entry_hash` under
///   `storage_key` for `source_id = keccak256(caller ‖ salt)` at the current EIP-7843
///   slot. Byte for byte the runtime of ethereum/sys-asm#53.
/// - **validate**, `n × 72` bytes of `source_id ‖ uint64_be(slot) ‖ root` tuples,
///   `1 ≤ n ≤ 16`, zero value: every tuple must name a slot strictly before the
///   current one and at most `RECENT_ROOT_USABLE_WINDOW` slots old, and its entry
///   hash must be the value stored under its storage key. Any failing tuple reverts;
///   success returns nothing and touches no state, which is what lets a validation
///   prefix be permitted to run it (`SLOTNUM` and its own `SLOAD`s only, no calls).
/// - anything else, or a non-zero value: revert.
///
/// The spec's two prohibitions on the write need no explicit check: a static context
/// fails on the `SSTORE` itself, and under `DELEGATECALL`/`CALLCODE` the write lands
/// in the calling account's storage, leaving recent-root storage untouched.
///
/// Provisional: the spec's constants table still reads `TBD` for `RECENT_ROOT_CODE`.
/// The validation operation is ethrex's implementation of the spec's prose, kept in
/// `scripts/hegota-testnet/recent_root/main.eas` (geas), and is recorded as a
/// divergence until upstream publishes bytes.
///
/// The code derives every ring index as `slot AND 0x1fff` (`push2 0x1fff`), which
/// equals `slot mod RECENT_ROOT_LENGTH` only while `RECENT_ROOT_LENGTH` is that power of
/// two, and compares ages against `push2 0x1fff` as `RECENT_ROOT_USABLE_WINDOW`. The
/// assertions below pin both constants the immediates assume.
pub const RECENT_ROOT_RUNTIME_BYTECODE: [u8; 345] = [
    ,
];
...
const _: () = assert!(
    ethrex_common::types::FRAME_TX_RECENT_ROOT_USABLE_WINDOW == 0x1fff,
    "RECENT_ROOT_RUNTIME_BYTECODE compares tuple ages against 0x1fff as RECENT_ROOT_USABLE_WINDOW"
);
...
mod expiry_verifier_tests {
    ...
    fn recent_root_constants_match_spec() {
        assert_eq!(RECENT_ROOT_RUNTIME_BYTECODE.len(), 345);
        assert_eq!(RECENT_ROOT_ADDRESS.address, H160::from_low_u64_be(0x8272));
        assert_eq!(RECENT_ROOT_ADDRESS.active_since_fork, Hegota);
    }

    /// The runtime is a byte string with no compiler behind it, so the whole body
    /// is hashed and the four domain immediates (two per operation) are checked
    /// against the preimages the read side derives in
    /// `RecentRootReference::{entry_hash, storage_key}`. A transcription error in
    /// any would otherwise surface only as a storage-key mismatch at validation.
    #[test]
    fn recent_root_bytecode_matches_spec() {
        assert_eq!(
            keccak(RECENT_ROOT_RUNTIME_BYTECODE),
            H256::from_slice(
                &hex::decode("cd1cae00e1d37cf97195f9e716dfa1b9a804e36bb5d7726c4f2c50e2580275a5")
                    .expect("code hash literal is valid hex")
            )
        );

        // The validate loop's immediates, then the write operation's.
        for (entry_at, storage_at) in [(0x28usize, 0x4cusize), (0xeb, 0x123)] {
            assert_eq!(RECENT_ROOT_RUNTIME_BYTECODE[entry_at], 0x7f);
            assert_eq!(
                &RECENT_ROOT_RUNTIME_BYTECODE[entry_at + 1..entry_at + 33],
                keccak(b"RECENT_ROOT_ENTRY").as_bytes()
            );
            assert_eq!(RECENT_ROOT_RUNTIME_BYTECODE[storage_at], 0x7f);
            assert_eq!(
                &RECENT_ROOT_RUNTIME_BYTECODE[storage_at + 1..storage_at + 33],

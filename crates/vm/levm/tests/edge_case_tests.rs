...
use ethrex_levm::{
    errors::{ExecutionResult, VMError},
...
fn test_extcodecopy_memory_allocation() {
    ...
    vm.run_execute(&mut current_call_frame).unwrap();
}
...
fn test_non_compliance_returndatacopy() {
    ...
    let mut current_call_frame = vm.call_frames.pop().unwrap();
    let txreport = vm.run_execute(&mut current_call_frame).unwrap();
    assert_eq!(
        txreport.result,
        ExecutionResult::Revert(VMError::OutOfBounds)
    );
}

...
impl<'a> VM<'a> {
    ...
    pub fn op_add(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        cur_frame.increase_consumed_gas(gas_cost::ADD)?;

        let [augend, addend] = *cur_frame.stack.pop()?;
        let sum = augend.overflowing_add(addend).0;
        cur_frame.stack.push1(sum)?;
    ...
    pub fn op_sub(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::SUB)?;

        let [minuend, subtrahend] = *cur_frame.stack.pop()?;
        let difference = minuend.overflowing_sub(subtrahend).0;
        cur_frame.stack.push1(difference)?;
    ...
    pub fn op_mul(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::MUL)?;

        let [multiplicand, multiplier] = *cur_frame.stack.pop()?;
        let product = multiplicand.overflowing_mul(multiplier).0;
        cur_frame.stack.push1(product)?;
    ...
    pub fn op_div(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::DIV)?;

        let [dividend, divisor] = *cur_frame.stack.pop()?;
        let Some(quotient) = dividend.checked_div(divisor) else {
            cur_frame.stack.push1(U256::zero())?;
            return Ok(OpcodeResult::Continue { pc_increment: 1 });
        };
        cur_frame.stack.push1(quotient)?;
    ...
    pub fn op_sdiv(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::SDIV)?;
    ...
    pub fn op_mod(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::MOD)?;

        let [dividend, divisor] = *cur_frame.stack.pop()?;

        let remainder = dividend.checked_rem(divisor).unwrap_or_default();

        cur_frame.stack.push1(remainder)?;
    ...
    pub fn op_smod(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::SMOD)?;

        let [unchecked_dividend, unchecked_divisor] = *cur_frame.stack.pop()?;
    ...
    pub fn op_addmod(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::ADDMOD)?;

        let [augend, addend, modulus] = *cur_frame.stack.pop()?;

        if modulus.is_zero() {
            cur_frame.stack.push(&[U256::zero()])?;
            return Ok(OpcodeResult::Continue { pc_increment: 1 });
        }
    ...
    pub fn op_mulmod(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::MULMOD)?;

        let [multiplicand, multiplier, modulus] = *cur_frame.stack.pop()?;
        ...
        cur_frame.stack.push1(product_mod)?;
    ...
    pub fn op_exp(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        let [base, exponent] = *cur_frame.stack.pop()?;

        let gas_cost = gas_cost::exp(exponent)?;

        cur_frame.increase_consumed_gas(gas_cost)?;

        let power = base.overflowing_pow(exponent).0;
        cur_frame.stack.push1(power)?;
    ...
    pub fn op_signextend(&mut self) -> Result<OpcodeResult, VMError> {
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::SIGNEXTEND)?;

        let [byte_size_minus_one, value_to_extend] = *cur_frame.stack.pop()?;

        if byte_size_minus_one > U256::from(31) {
            cur_frame.stack.push1(value_to_extend)?;
            return Ok(OpcodeResult::Continue { pc_increment: 1 });
        }

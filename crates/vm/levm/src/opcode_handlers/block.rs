...
impl<'a> VM<'a> {
    ...
    pub fn op_blockhash(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        cur_frame.increase_consumed_gas(gas_cost::BLOCKHASH)?;

        let block_number = cur_frame.stack.pop1()?;
    ...
    pub fn op_coinbase(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::COINBASE)?;

        cur_frame.stack.push1(address_to_word(coinbase))?;
    ...
    pub fn op_timestamp(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::TIMESTAMP)?;

        cur_frame.stack.push1(timestamp)?;
    ...
    pub fn op_number(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::NUMBER)?;

        cur_frame.stack.push1(block_number)?;
    ...
    pub fn op_prevrandao(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::PREVRANDAO)?;
        cur_frame.stack.push1(randao)?;
    ...
    pub fn op_gaslimit(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::GASLIMIT)?;

        cur_frame.stack.push1(block_gas_limit.into())?;
    ...
    pub fn op_chainid(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::CHAINID)?;

        cur_frame.stack.push1(chain_id)?;
    ...
    pub fn op_selfbalance(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let balance = self.db.get_account(self.cur_frame()?.to)?.info.balance;

        self.cur_frame_mut()?.stack.push1(balance)?;
        Ok(OpcodeResult::Continue { pc_increment: 1 })
    }
    ...
    pub fn op_basefee(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let cur_frame = self.cur_frame_mut()?;
        cur_frame.increase_consumed_gas(gas_cost::BASEFEE)?;

        cur_frame.stack.push1(base_fee_per_gas)?;
    ...
    pub fn op_blobhash(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        let index = self.cur_frame_mut()?.stack.pop1()?;
        ...
        let index: usize = match index.try_into() {
            Ok(index) => index,
            Err(_) => {
                self.cur_frame_mut()?.stack.push1(U256::zero())?;
                return Ok(OpcodeResult::Continue { pc_increment: 1 });
            }
        };
        ...
        self.cur_frame_mut()?.stack.push1(hash)?;
    ...
    pub fn op_blobbasefee(&mut self) -> Result<OpcodeResult, VMError> {
        ...
        self.cur_frame_mut()?.stack.push1(blob_base_fee)?;

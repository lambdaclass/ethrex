...
impl<'a> VM<'a> {
    ...
    pub fn op_call(&mut self) -> Result<OpcodeResult, VMError> {
        let (
            gas, callee, value_to_transfer, current_memory_size,
            args_start_offset, args_size, return_data_start_offset,  return_data_size
        ) = {
        ...
        self.current_call_frame_mut()?.increase_consumed_gas(eip7702_gas_consumed)?;
    ...
    pub fn op_callcode(
        ...
        // STACK
        let (
            gas, code_address, value_to_transfer, current_memory_size,
            args_start_offset, args_size, return_data_start_offset, return_data_size
        ) = {
            let current_call_frame = self.current_call_frame_mut()?;
            let gas = current_call_frame.stack.pop()?;
    ...
    pub fn op_delegatecall(
        ...
        // STACK
        let (
            gas, code_address, current_memory_size,
            args_start_offset, args_size, return_data_start_offset, return_data_size
        ) = {
            let current_call_frame = self.current_call_frame_mut()?;
            let gas = current_call_frame.stack.pop()?;
    ...
    pub fn op_selfdestruct(
        ...
        self.current_call_frame_mut()?.increase_consumed_gas(gas_cost::selfdestruct(
    ...
    pub fn generic_create(
        ...
        if new_account.has_code_or_nonce() {
            ...
            self.current_call_frame_mut()?.stack.push(CREATE_DEPLOYMENT_FAIL)?;
            return Ok(OpcodeResult::Continue { pc_increment: 1 });
        }
    ...
    pub fn handle_return_create(&mut self, tx_report: &ExecutionReport, retdata: RetData) -> Result<(), VMError>{
        let unused_gas = retdata.max_message_call_gas
            .checked_sub(tx_report.gas_used)
            .ok_or(InternalError::GasOverflow)?;

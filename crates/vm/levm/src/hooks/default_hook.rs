...
pub fn undo_value_transfer(vm: &mut VM<'_>) -> Result<(), VMError> {
    ...
    if !vm.is_create()? {
        vm.decrease_account_balance(vm.cur_frame()?.to, vm.cur_frame()?.msg_value)?;
    }

    vm.increase_account_balance(vm.env.origin, vm.cur_frame()?.msg_value)?;
...
pub fn validate_min_gas_limit(vm: &mut VM<'_>) -> Result<(), VMError> {
    ...
    let calldata = vm.cur_frame()?.calldata.clone();
    ...
    if vm.cur_frame()?.gas_limit < min_gas_limit {
...
pub fn validate_init_code_size(vm: &mut VM<'_>) -> Result<(), VMError> {
    ...
    let code_size = vm.cur_frame()?.calldata.len();
...
pub fn validate_sender_balance(vm: &mut VM<'_>, sender_balance: U256) -> Result<(), VMError> {
    ...
    let value = vm.cur_frame()?.msg_value;
...
pub fn transfer_value(vm: &mut VM<'_>) -> Result<(), VMError> {
    if !vm.is_create()? {
        vm.increase_account_balance(vm.cur_frame()?.to, vm.cur_frame()?.msg_value)?;
    }
    Ok(())
}
...
pub fn set_bytecode_and_code_address(vm: &mut VM<'_>) -> Result<(), VMError> {
    ...
    let (bytecode, code_address) = if vm.is_create()? {
        ...
        let calldata = std::mem::take(&mut vm.cur_frame_mut()?.calldata);
        (calldata, vm.cur_frame()?.to)
    } else {
        // Here bytecode and code_address could be either from the account or from the delegated account.
        let to = vm.cur_frame()?.to;
    ...
    vm.cur_frame_mut()?.code_address = code_address;
    vm.cur_frame_mut()?.set_code(bytecode)?;

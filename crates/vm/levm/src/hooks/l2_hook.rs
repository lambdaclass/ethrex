...
use super::{
    default_hook::{DefaultHook, MAX_REFUND_QUOTIENT, MAX_REFUND_QUOTIENT_PRE_LONDON},
    hook::Hook,
};
...
impl Hook for L2Hook {
    fn prepare_execution(
        ...
        //     )?;
        // }
        let default_hook = DefaultHook;
        match default_hook.prepare_execution(vm, initial_call_frame) {
            Ok(_) => (),
            Err(e) => {
                println!("Error in default hook: {:?}", e);
                increase_account_balance(
                    &mut vm.cache,
                    vm.db.clone(),
                    initial_call_frame.to,

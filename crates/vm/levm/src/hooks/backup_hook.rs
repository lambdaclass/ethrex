...
impl Hook for BackupHook {
    fn prepare_execution(&mut self, vm: &mut crate::vm::VM<'_>) -> Result<(), VMError> {
        ...
        self.pre_execution_backup = vm.current_call_frame.call_frame_backup.clone(); // ok-clone: we need the original copies of the values later
        Ok(())
    }
    ...
    fn finalize_execution(
        ...
        let mut execution_backup = vm.current_call_frame.call_frame_backup.clone(); // ok-clone: we need the original copies of the values later

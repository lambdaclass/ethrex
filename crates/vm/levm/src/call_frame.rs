...
impl CallFrame {
    ...
    pub fn restore_cache_state(&self, db: &mut GeneralizedDatabase) -> Result<(), VMError> {
        for (address, account) in self.call_frame_backup.original_accounts_info.clone() {
        ...
        for (address, storage) in self.call_frame_backup.original_account_storage_slots.clone() {

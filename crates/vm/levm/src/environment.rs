...
pub struct Environment {
    ...
    /// When true, skip the tx nonce-mismatch check. Used only by the speculative block
    /// warmer: heavy same-sender groups are split and warmed from parent state, so the
    /// tail txs carry a nonce ahead of the parent account nonce and would otherwise abort
    /// before their bodies (and read-sets) execute.

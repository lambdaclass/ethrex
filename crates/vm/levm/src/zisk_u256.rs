...
// correctness q*b + r == a and r < b are checked by assert.
...
/// `add256` writing into a caller-provided `out`, with operands passed BY
/// REFERENCE. `overflowing_add(a, b)` takes both operands by value, forcing LLVM
/// to `memcpy` them into fresh aligned slots just to hand the syscall `&a.0`/`&b.0`
/// (a constrained AIR call body on riscv64ima). Pointing the syscall directly at
/// the live stack slots + a distinct `out` deletes both copies. Returns the carry.
#[inline(always)]
pub fn add_into(a: &[u64; 4], b: &[u64; 4], out: &mut [u64; 4]) -> bool {
    ...
    syscall_add256(&mut params) != 0
}

/// `a - b` (= `a + !b + 1`) into `out`, by reference (see [`add_into`]). Returns
/// whether the subtraction did NOT borrow (matches `overflowing_sub`'s bool).
#[inline(always)]
pub fn sub_into(a: &[u64; 4], b: &[u64; 4], out: &mut [u64; 4]) -> bool {

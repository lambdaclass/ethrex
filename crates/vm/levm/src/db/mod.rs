...
// contention. DashMap shards by key hash so workers touching different
// addresses don't contend.
...
///
/// Thread-safe via sharded concurrent maps (DashMap) — optimized for highly
/// concurrent access from rayon workers in the parallel BAL path.
...
impl Database for CachingDatabase {
    ...
    fn prefetch_accounts(&self, addresses: &[Address]) -> Result<(), DatabaseError> {
        // Fetch from inner in parallel and insert directly (DashMap shards by key).
        addresses
            .par_iter()

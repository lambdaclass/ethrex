...
///
/// Thread-safe via DashMap - a concurrent hash map that provides fine-grained
/// locking at the shard level, avoiding the read-write lock contention that
/// occurs with RwLock<HashMap> under concurrent access patterns.
///
...
impl Database for CachingDatabase {
    fn get_account_state(&self, address: Address) -> Result<AccountState, DatabaseError> {
        // Check cache first - DashMap provides lock-free reads for existing entries
        ...
        // Populate cache - DashMap handles concurrent inserts safely

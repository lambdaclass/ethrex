...
pub(crate) struct OpCodeFn<'a>(fn(&'_ mut VM<'a>, error: &mut OnceCell<VMError>) -> OpcodeResult);

impl<'a> OpCodeFn<'a> {
    /// Call the opcode handler.
    #[inline(always)]
    pub fn call(self, vm: &mut VM<'a>, error: &mut OnceCell<VMError>) -> OpcodeResult {
        (self.0)(vm, error)
    }
}
...
impl<'a> VM<'a> {
    ...
    pub fn on_invalid_opcode(&mut self, error: &mut OnceCell<VMError>) -> OpcodeResult {
        error.set(ExceptionalHalt::InvalidOpcode.into());
        OpcodeResult::Halt
    }

    pub fn op_stop(&mut self, _error: &mut OnceCell<VMError>) -> OpcodeResult {
        OpcodeResult::Halt
    }
}
